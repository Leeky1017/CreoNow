...
- [ ] 2.3 收集并回填各执行子任务的 Red/Green 证据到 ISSUE-626 RUN_LOG

## 3. Red（先写失败测试）

- [ ] 3.1 记录 Workbench 提质相关 Red 失败测试输出（Scroll/Motion/A11y/Visual）
- [ ] 3.2 记录 Editor 提质相关 Red 失败测试输出（Typography/Scroll/Motion/A11y/Visual）

## 4. Green（最小实现通过）

- [ ] 4.1 回填 Workbench 对应 Green 通过证据
- [ ] 4.2 回填 Editor 对应 Green 通过证据
- [ ] 4.3 记录 required checks 对齐策略与执行结果（`ci` / `openspec-log-guard` / `merge-serial`）

## 5. Refactor（保持绿灯）

- [ ] 5.1 回填去重/收敛改动后的回归验证证据
