---
description: 'Repair failing CreoNow CI on an existing task branch / PR without breaking 1+4+1 continuity.'
---
...
7. 修复完成后回到 1+4+1 审计循环：四审并行、任一 finding 回修、四审全 `FINAL-VERDICT: ACCEPT` 且 zero findings 才可收口。
8. 默认不启用 auto-merge。
