---
description: 'Create or update a CreoNow delivery PR with the 1+4+1 audit-first GitHub flow.'
---

请按 CreoNow 的 1+4+1 交付链路完成任务，不要把最后一步留给用户手工补。
...
4. 审计必须是 1+4+1：四个独立全量审计 + 一个 reviewer 单条汇总评论。
5. 任一审计 finding 即回工程修复并重跑四审。
6. 默认不启用 auto-merge；仅在四审全部 `FINAL-VERDICT: ACCEPT` 且 zero findings 后才可显式开启。
