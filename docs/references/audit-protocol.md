...
2. 同一轮实现必须先由工程 Subagent 完成，再进入审计阶段。
...
6. 只有两名审计都不再报告问题，且都给出 `FINAL-VERDICT` + `ACCEPT`，才能结束循环。
...
| `frontend` | ① Design Token 使用率（禁硬编码） ② 组件复用（检查 primitives/） ③ i18n `t()` 覆盖 ④ 交互状态过渡 ⑤ Storybook Story |
...
10. **不能**跳过 UI 协议检查——涉及 UI 改动的 PR 必须验证 Design Token 使用率、品牌一致性；PRE-AUDIT 必须包含至少 1 条产品行为验证
...
合并门槛：必须收集到两名独立审计 Agent 的 `FINAL-VERDICT` + `ACCEPT`，且两边都无未决问题。
