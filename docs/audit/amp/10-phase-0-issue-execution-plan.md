...
| A0-02 | 自动保存失败可见化 | 直接 PR | 待创建 | 待分配 | `autosave-visible-failure` | 需要 |
| A0-03 | 渲染进程全局错误兜底 | 直接 PR | 待创建 | 待分配 | `renderer-global-error-fallback` | 可选 |
...
| A0-06 | 发布事实表 | 直接 PR | 待创建 | 待分配 | `release-fact-sheet` | 否 |
| A0-07 | Windows 首发边界核查 | 直接 PR | 待创建 | 待分配 | `windows-release-boundary-audit` | 否 |
| A0-08 | 备份能力真伪核查 | 决策 issue | 待创建 | 待分配 | `backup-capability-decision` | 否 |
| A0-09 | i18n 存量 key 核查 | 直接 PR | 待创建 | 待分配 | `i18n-inventory-audit` | 需要 |
| A0-10 | 基础全文搜索入口 | OpenSpec Change | 已创建 | #1003 | `search-mvp` | 需要 |
| A0-11 | 数据安全边界声明 | 直接 PR | 待创建 | 待分配 | `data-safety-boundary-statement` | 否 |
| A0-12 | Inline AI 从 0 到 1 新建 | OpenSpec Change | 已创建 | #1004 | `inline-ai-baseline` | 需要 |
| A0-13 | Toast 接入 App | 直接 PR | 待创建 | 待分配 | `toast-app-integration` | 需要 |
| A0-14 | Settings General 持久化 | 直接 PR | 待创建 | 待分配 | `settings-general-persistence` | 需要 |
| A0-15 | 占位 UI 收口 | 直接 PR | 待创建 | 待分配 | `placeholder-ui-closure` | 需要 |
| A0-16 | 编辑器/版本/Slash i18n 核查 | 直接 PR | 待创建 | 待分配 | `editor-version-slash-i18n` | 需要 |
| A0-17 | Backup 决策：实现或隐藏 | 决策 issue | 待创建 | 待分配 | `backup-entry-resolution` | 视结论而定 |
| A0-18 | Judge 决策：接入或降级 | 决策 issue | 待创建 | 待分配 | `judge-capability-resolution` | 视结论而定 |
| A0-19 | Export 纯文本诚实标注 | 直接 PR | 待创建 | 待分配 | `export-plain-text-labeling` | 需要 |
| A0-20 | 错误消息统一人话化 | 直接 PR | 待创建 | 待分配 | `error-message-humanization` | 否 |
| A0-21 | 错误展示组件收口 | 直接 PR | 待创建 | 待分配 | `error-surface-closure` | 需要 |
| A0-22 | i18n 错误文案修正 | 直接 PR | 待创建 | 待分配 | `i18n-error-copy-cleanup` | 需要 |
| A0-23 | 文档 5MB 限制实施 | 直接 PR | 待创建 | 待分配 | `document-size-limit-enforcement` | 否 |
| A0-24 | Skill 输出校验扩展 | 直接 PR | 待创建 | 待分配 | `skill-output-validation` | 否 |
...
|------|-------------|------------|------|
| A0-01 | `openspec/changes/a0-01-zen-mode-editable` | 已有关联 issue | 见 `EXECUTION_ORDER.md` |
| A0-04 | `openspec/changes/a0-04-export-honest-grading` | 已有关联 issue | 见 `EXECUTION_ORDER.md` |
| A0-05 | `openspec/changes/a0-05-skill-router-negation-guard` | 已有关联 issue | 见 `EXECUTION_ORDER.md` |
| A0-10 | `openspec/changes/a0-10-search-mvp` | 已有关联 issue | 见 `EXECUTION_ORDER.md` |
| A0-12 | `openspec/changes/a0-12-inline-ai-baseline` | 已有关联 issue | 见 `EXECUTION_ORDER.md` |

### 3.3 还需要创建的 issue 数量

截至本文更新时间：

- 已有 active change 对应 issue：5
- 待创建主 issue：19
- 其中“决策 issue”：3
  - A0-08：备份能力真伪核查
  - A0-17：Backup 最终处置
...
说明：A0-08 与 A0-17 不是重复。A0-08 负责查清事实与约束，A0-17 负责基于事实做产品决策并收口到代码或 UI。
...
Issue 创建顺序要先满足“上下游可见”，再满足“并行效率”。因此，所有前置决策与基础设施 issue 应先创建，即使实现排期不一定最先开始。

### 4.2 建议的 6 个创建批次

| 批次 | 先创建哪些 issue | 目的 | 创建后允许的动作 |
|------|------------------|------|------------------|
| Batch 0 | A0-08, A0-17, A0-18 | 先把 Backup/Judge 的歧义收口 | 决策调研、文档核查、选项比较 |
| Batch 1 | A0-13, A0-20, A0-23, A0-24 | 打基础设施地基 | 可立即开工 |
| Batch 2 | A0-02, A0-03, A0-14, A0-15 | 接入反馈链路与 UI 收口 | 等 Batch 1 至少进入 Red |
| Batch 3 | A0-09, A0-16, A0-21, A0-22 | i18n 与错误展示收口 | 依赖 A0-20，A0-16 依赖 A0-09 |
| Batch 4 | A0-06, A0-07, A0-11, A0-19 | 文档与诚实标注收口 | A0-19 依赖 A0-04 change 结果 |
| Batch 5（无需新建） | A0-01, A0-04, A0-05, A0-10, A0-12 | 这 5 项已有关联 issue 与 active change，本表仅做登记提示 | 跟踪 active changes |

说明：Batch 5 不计入“待创建的 19 个主 issue”。

### 4.3 创建与实现的推进时机规则

- Batch 0 创建后，可立即开始事实调研与方案对比。
- Batch 1 创建后，可立即分配执行 Agent 开始实现。
- Batch 2 及以后创建后，必须先检查其上游项是否已进入实现或已合并回 `main`。
...
5. 需要复用已关闭 issue 或非 `task/*` 分支才能继续。
