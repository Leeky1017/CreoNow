...
| 四 | 文档清单 | 各文件分工 |
| 五 | 总体判断 | 本轮综合整理后的核心结论 |
| 六 | GitHub 执行入口（后端治理专题） | 4 条母 Issue 与 Amp 路线图的映射关系 |
| 七 | 维护约定 | 后续如何持续更新本专题 |
...
## 六、GitHub 执行入口（后端治理专题，2026-03-06 重构）

为避免 OpenClaw / Asuka / Checa 历史 issue 在 GitHub 中碎片化悬挂，本专题已把**后端治理专题**相关问题收束为 4 条 **母 Issue**。

它们的作用不是替代 Amp 文档，而是把 Amp 已识别的后端治理风险，转译成 GitHub 上可以持续推进、持续验收、持续归档的执行容器。

它们**不覆盖** Phase 0 的单项任务 issue，也**不取代**产品、前端或纯文档治理线的执行入口。
...
截至本次重构后，GitHub 上与本专题**后端治理专题**对应的 open 执行入口，应以这 4 条母 Issue 为准；Phase 0 的单项任务仍应回到各自 issue 执行。
