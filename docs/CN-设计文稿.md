...
| **AI FAB** | 未实现 | 已实现（`AppLayout.tsx` 中 `button`  • `onMouseDown` 拖拽。**仅拖拽**，无单击/双击事件绑定） | 移除拖拽，新增单击/双击/长按交互（见 §5） |
...
<AppLayout>                          ← 顶层布局，持有 isZenMode / isFocusMode 状态
  ...
  <AIFab/>                          ← 浮动按钮（固定右下角）；隐藏于 Zen/Focus Mode
...
> **Note**: 此 §23 涵盖 App Shell 骨架层。间距系统的 §23 编号为历史遗留（Notion 原始区块编号），实际内容见后续独立章节。两个 §23 在 SSOT 中已分拆为 [20.12 App Shell](cn-frontend-ssot/20-design-v3/20.12-app-shell.md) 和 [20.5 间距系统](cn-frontend-ssot/20-design-v3/20.5-spacing.md)，不再有歧义。
>
> **位置说明**: 此 §23（App Shell 与布局子系统）在原稿中位于 §16 和 §17 之间，属于历史编辑遗留的位置错位。逻辑上应在 §22 之后。SSOT 中已按职责正确拆分，此处保留原始位置不做移动。
...
| `--shadow-xl` | `0 16px 48px rgba(0,0,0,0.7)` | Layer 6 | Toast |
...
t=420ms  AI FAB scale(0)→1 + opacity 0→1
