...
- If `selected_channel=gh`, continue using the local `gh`-based delivery scripts, but keep auto-merge off by default; only pass `--enable-auto-merge` after the Reviewer has posted the single consolidated zero-findings audit comment.
