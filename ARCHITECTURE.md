...
CN 的记忆架构分三层（目标设计）。默认走 Layer 0+1，只有 0+1 无法回答时才升级到 Layer 2。当前实现基于 SQLite（`user_memory` 表存储用户记忆，`memory_episodes` / `memory_semantic_placeholders` 表存储情景记忆，CRUD 通过 `services/memory/memoryService.ts`）。下述 Markdown 文件方案为計劃实现，尚未落地。KG + FTS5 是主检索路径；RAG（向量检索 + embedding）作为补充手段存在（`services/rag/`、`services/embedding/`），但不作为默认检索策略。（目标约束：逐步收敛到 KG+FTS5 优先，RAG 仅在精度不足时降级使用）
