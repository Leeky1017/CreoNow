...
CN 的记忆架构分三层（目标设计）。默认走 Layer 0+1，只有 0+1 无法回答时才升级到 Layer 2。当前实现基于 SQLite（`user_memory` 表存储用户记忆，CRUD 通过 `services/memory/memoryService.ts`；`memory_episodes` / `memory_semantic_placeholders` 表存储情景记忆，CRUD 通过 `services/memory/episodicMemoryService.ts`）。下述 Markdown 文件方案为計畫实现，尚未落地。KG + FTS5 是唯一检索路径。
