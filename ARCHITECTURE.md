...
CN 的记忆架构分三层（目标设计）。默认走 Layer 0+1，只有 0+1 无法回答时才升级到 Layer 2。当前实现基于 SQLite（`user_memory` 表存储用户记忆，CRUD 通过 `services/memory/memoryService.ts`；`memory_episodes` / `memory_semantic_placeholders` 表存储情景记忆，CRUD 通过 `services/memory/episodicMemoryService.ts`）。下述 Markdown 文件方案为計劃实现，尚未落地。KG + FTS5 是主检索路径；RAG（向量检索 + embedding）作为补充手段存在（`services/rag/`、`services/embedding/`），但不作为默认检索策略。（目标约束：逐步收敛到 KG+FTS5 优先，RAG 仅在精度不足时降级使用）

**Layer 0 -- 始终注入（Always-Inject Core）**（計劃实现，当前 Layer 0 通过 SQLite `user_memory` 表实现）

- 内容：蒸馏后的用户事实 + 行为洞察 + 活跃项目核心设定
- 存储：目标方案为 `MEMORY.md`（<=200 行 / 25KB）+ `PROJECT_CONTEXT.md`（项目级）（均为計劃创建，尚未存在；当前存储在 SQLite `user_memory` 表）
- 注入方式：每次对话全量注入 system prompt，不搜索、不检索
- 预算：硬性上限（如 3000 tokens），超出则触发整合

**Layer 1 -- 会话感知注入（Session-Aware Injection）**（計劃实现）

- 内容：近期对话摘要 + 今日工作笔记 + 当前活跃章节的 KG 实体摘要
- 存储：目标方案为 `memory/YYYY-MM-DD.md`（每日笔记，計劃创建，尚未存在）+ KG 摘要缓存
...
每次 AI 调用记录：模型名称、input/output tokens、估算费用（USD）。`recordUsage()` 接口支持可选的 `cachedTokens` 参数，但当前 orchestrator 调用链路尚未传入该值（默认为 0）。主进程 in-memory Map 追踪（maxRecords=500 自动淘汰）。IPC handler 已注册，渲染进程 UI（计划实现）。
