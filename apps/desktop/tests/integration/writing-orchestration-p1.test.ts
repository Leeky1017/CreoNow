...
import { createToolRegistry, buildTool } from "../../main/src/services/skills/toolRegistry";
import { estimateTokens } from "../../main/src/services/context/tokenEstimation";
import { estimateCjkAwareTokenCount } from "@shared/tokenBudget";
import type { StreamChunk } from "../../main/src/services/ai/streaming";
  ...
  it("abort during AI streaming halts pipeline cleanly", async () => {
    ...
    const mockAiService = {
      ...
      async *streamChat(
        ...
        // Abort immediately after AI starts
        ctrl.abort();
        if (opts.signal.aborted) return;
    ...
    orchestrator.abort(request.requestId); // pre-abort
    const events = await collectEvents(orchestrator.execute(request));
    const types = events.map((e) => e.type);

    assert.ok(
      types.includes("error") || types.includes("aborted"),
      `expected error or aborted, got: ${types.join(", ")}`,
    );
...
describe("P1 context layers — Rules + Immediate only", () => {
  it("assembleContextLayers produces Rules and Immediate layers, settings and retrieved disabled", () => {
    ...
    const { assembleContextLayers } = require("../../main/src/services/context/tokenEstimation");
