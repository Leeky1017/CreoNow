...
export async function runReplaceVersionSnapshotScenario(): Promise<void> {
  ...
  const manualSaveVersion1 = doc1Versions.find(
    (version) => version.reason === "manual-save-1",
  );
  const manualSaveVersion2 = doc1Versions.find(
    (version) => version.reason === "manual-save-2",
  );
  const preSearchReplaceVersion = doc1Versions.find(
    (version) => version.reason === "pre-search-replace",
  );
  const searchReplaceVersion = doc1Versions.find(
    (version) => version.reason === "search-replace",
  );
  ...
  assert.equal(
    searchReplaceVersion?.parentVersionId,
    preSearchReplaceVersion?.versionId ?? null,
  );
