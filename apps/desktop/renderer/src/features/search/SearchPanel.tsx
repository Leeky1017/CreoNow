...
export const MOCK_SEARCH_RESULTS: SearchResultItem[] = [
  {
    ...
    snippet: "...visual noise. In design theory, this absence of clutter allows the user to focus purely on function. The aesthetic of silence is not empty...",
  ...
  },
  {
    ...
    snippet: "...exploring the roots of design theory through the lens of early 20th century functionalism...",
...
function ToggleSwitch(props: {
  ...
  return (
    <label htmlFor={props.id} className="flex items-center gap-2 cursor-pointer group">
...
function DocumentResultItem(props: {
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      <div className={`mt-1 w-8 h-8 rounded flex items-center justify-center border border-[rgba(255,255,255,0.05)] shrink-0 transition-colors ${
        isActive 
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={`text-xs leading-relaxed transition-colors ${
          isActive ? "text-[#888888] line-clamp-2" : "text-[#444444] group-hover:text-[#888888] line-clamp-1"
        }`}>
          <HighlightText text={item.snippet || ""} query={query} />
        </p>
        {item.path && (
          <div className="flex items-center gap-2 mt-2">
            <svg className="w-3 h-3 text-[#444444]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            {item.editedTime && (
              <>
                ...
                <span className="text-[10px] text-[#444444]">{item.editedTime}</span>
              </>
            )}
          </div>
        )}
...
function ResultGroup(props: {
  ...
  return (
    <div className={`py-2 ${props.hasBorderTop ? "border-t border-[rgba(255,255,255,0.04)]" : ""}`}>
 ...
 */
function KeyHint(props: { icon?: React.ReactNode; text?: string; label: string }): JSX.Element {
  return (
...
export function SearchPanel(props: {
  ...
  const items: SearchResultItem[] = mockResults || storeItems.map((item) => ({
    id: item.documentId,
    ...
    >
      ...
      <div 
        ...
        style={{
          ...
          boxShadow: "0 24px 48px -12px rgba(0, 0, 0, 0.7), 0 0 0 1px rgba(255, 255, 255, 0.02) inset",
          animation: "slideDown 0.3s ease-out",
        }}
      >
        {/* Header */}
        <div className="flex flex-col border-b border-[rgba(255,255,255,0.06)]" style={{ background: "#0f0f0f" }}>
          {/* Search input row */}
          ...
          <div className="flex items-center gap-2 px-4 pb-4 overflow-x-auto">
            ...
            <CategoryButton label="Documents" active={category === "documents"} onClick={() => setCategory("documents")} />
            <CategoryButton label="Memories" active={category === "memories"} onClick={() => setCategory("memories")} />
            <CategoryButton label="Knowledge" active={category === "knowledge"} onClick={() => setCategory("knowledge")} />
            <CategoryButton label="Assets" active={category === "assets"} onClick={() => setCategory("assets")} />
          </div>
        ...
        <div className="flex-1 overflow-y-auto" style={{ background: "#0f0f0f" }}>
          {!hasQuery && !hasResults ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 px-8">
              <svg className="w-16 h-16 text-[#444444] mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={0.75}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            ...
            <>
              {/* Documents group */}
              {documentItems.length > 0 && (category === "all" || category === "documents") && (
              ...
              {/* Memories group */}
              {memoryItems.length > 0 && (category === "all" || category === "memories") && (
                <ResultGroup 
                  title="Memories" 
              ...
              {/* Knowledge group */}
              {knowledgeItems.length > 0 && (category === "all" || category === "knowledge") && (
                <ResultGroup 
                  title="Knowledge Graph" 
                  count={knowledgeItems.length}
                  hasBorderTop={documentItems.length > 0 || memoryItems.length > 0}
                >
