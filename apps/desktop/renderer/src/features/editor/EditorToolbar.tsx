...
export function EditorToolbar({
  ...
  return (
    ...
    >
      ...
      {/* Overflow "More" menu — visible when toolbar buttons exceed container width */}
      {isOverflowing ? (
        <div className="relative ml-auto flex-shrink-0">
          ...
          {overflowMenuOpen ? (
            ...
            >
              <ToolbarButton
                testId="overflow-undo"
                label={EDITOR_SHORTCUTS.undo.label}
                shortcut={EDITOR_SHORTCUTS.undo.display()}
                disabled={disabled || !editor.can().undo()}
                onClick={() => { editor.chain().focus().undo().run(); setOverflowMenuOpen(false); }}
              >
                {icons.undo}
              </ToolbarButton>
              <ToolbarButton
                testId="overflow-redo"
                label={EDITOR_SHORTCUTS.redo.label}
                shortcut={EDITOR_SHORTCUTS.redo.display()}
                disabled={disabled || !editor.can().redo()}
                onClick={() => { editor.chain().focus().redo().run(); setOverflowMenuOpen(false); }}
              >
                {icons.redo}
              </ToolbarButton>
            </div>
