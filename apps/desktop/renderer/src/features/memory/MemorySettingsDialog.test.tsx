...
describe("MemorySettingsDialog", () => {
  ...
  describe("渲染", () => {
    it("打开时应该渲染 Dialog", () => {
      render(
        <MemorySettingsDialog open={true} onOpenChange={vi.fn()} />
      );
    ...
    it("关闭时不应该渲染内容", () => {
      render(
        <MemorySettingsDialog open={false} onOpenChange={vi.fn()} />
      );
  ...
  describe("帮助文本", () => {
    ...
    it("应该显示学习开关的说明", () => {
      ...
      expect(screen.getByText("AI 会从你的反馈中学习写作偏好")).toBeInTheDocument();
    });
  ...
  describe("按钮", () => {
    ...
    it("点击完成按钮应该调用 onOpenChange(false)", async () => {
      ...
      render(
        <MemorySettingsDialog open={true} onOpenChange={onOpenChange} />
      );
