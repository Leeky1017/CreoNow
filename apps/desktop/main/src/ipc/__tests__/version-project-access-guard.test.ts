...
describe("version project access guard", () => {
  it("version:snapshot:create rejects mismatched bound projectId", async () => {
  ...
  it("version:snapshot:create fails closed when renderer session is unbound", async () => {
    const binding = createProjectSessionBindingRegistry();
  ...
  it("version list/read/rollback/restore enforce bound project session", async () => {
    const binding = createProjectSessionBindingRegistry();
