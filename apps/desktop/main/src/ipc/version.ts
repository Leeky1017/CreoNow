...
function registerVersionBranchHandlers(ctx: VersionHandlerContext): void {
  const { ipcMain, logger } = ctx;

  // P1 GATE: Version branching, merging, and conflict resolution are V2 features.
  // These handlers are intentionally hard-gated to prevent the P1 live surface
  // from exposing unfinished multi-branch workflows to users.
  const p1NotImplemented: IpcResponse<never> = {
    ok: false,
    error: {
      code: "UNSUPPORTED",
  ...
  ipcMain.handle("version:branch:create", async () => p1NotImplemented);
  ipcMain.handle("version:branch:list", async () => p1NotImplemented);
  ipcMain.handle("version:branch:switch", async () => p1NotImplemented);
  ipcMain.handle("version:branch:merge", async () => p1NotImplemented);
  ipcMain.handle("version:conflict:resolve", async () => p1NotImplemented);

  // version:aiapply:logconflict is kept as a lightweight audit log even in P1.
  ipcMain.handle(
