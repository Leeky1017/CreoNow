...
function resolveProjectIdColumn(db: Database.Database): "project_id" | "id" {
  const rows = db
...
export function createBackupSnapshot(
  ...
  const projectIdColumn = resolveProjectIdColumn(deps.db);
  const project = deps.db
    .prepare(`SELECT ${projectIdColumn} as projectId FROM projects WHERE ${projectIdColumn} = ?`)
