...
P3 阶段目标：从基础三技能扩展为**项目级智能辅助**——新增一致性检查、对白生成、大纲展开三个技能，AI 在项目上下文中工作。

| 变更 | 描述 |
|------|------|
| P3 — 一致性检查技能 | 检查角色/地点设定与正文的一致性，标记矛盾处 |
| P3 — 对白生成技能 | 根据角色设定和场景上下文生成对白 |
| P3 — 大纲展开技能 | 将大纲要点展开为完整段落或章节 |
| P3 — SKILL.md 定义格式 | 基于 Markdown frontmatter 的技能定义文件 |
...
| Skills   | `apps/desktop/main/skills/packages/`             |
...
interface WritingRequest {
  ...
  /**
   * P3 扩展：项目上下文技能需要此字段。
   * Orchestrator 可从 documentId 反查 projectId，但显式传入优先。
   * 当 SkillInputRequirement.requiresProjectContext 为 true 时，
   * 此字段应由调用方填入。
   */
  projectId?: string
}
...
- **并且** 调用 `tool.execute(ctx)` 成功后返回 `{ ok: true }`
...
interface SkillInputRequirement {
  ...
  /**
   * P3 扩展：是否需要项目级上下文（角色/地点设定）。
   * P1 技能此字段为 undefined（等同 false）。
   * P3 技能通过 SkillManifest.inputType 与 context_rules 声明，
   * 注册时映射到此字段。Orchestrator 据此决定是否查询 Settings 模块。
   */
...
- **则** Hook 返回 `{ ok: false, error: '磁盘空间不足' }`
...
- **当** 数据发送到主进程
- **则** Zod 校验失败，主进程返回 `{ ok: false, error: { code: "VALIDATION_ERROR", message: "promptTemplate 不能为空" } }`
...
- **则** 主进程通过 `skill:stream:done` 推送错误状态 `{ ok: false, error: { code: "LLM_API_ERROR", message: "API 调用失败，请稍后重试" } }`
...
> **注意**：P2 Agentic Loop 中 AI 可调用的工具**仅限只读工具**（kgTool、memTool、docTool）。`documentWrite` 不暴露给 AI 自主调用——文档写入必须走建议 → 用户确认流程。如 AI 请求调用 `documentWrite`，系统应返回 `{ ok: false, error: { code: 'TOOL_USE_TOOL_NOT_FOUND' } }`。

#### Scenario: Tool 执行结果注入消息流

- **假设** `kgTool` 返回 `{ ok: true, data: { traits: ['冷静', '理性'] } }`
...
- **则** 该 tool call 返回 `{ ok: false, error: { code: 'TOOL_USE_TOOL_NOT_FOUND', message: 'unknownTool 未注册' } }`
...
- **则** kgTool 查询知识图谱，返回 `{ ok: true, data: { name: '林远', traits: ['冷静', '理性', '偶尔冷幽默'] } }`
...
- **则** memTool 返回 `{ ok: true, data: { memories: [] } }`（空结果，非错误）
...
> **阶段**: P3（项目制长篇创作）
> **CC 参考**: `skills/` SKILL.md frontmatter 解析 + skill registration dispatch

### Requirement: P3 — SKILL.md 技能定义格式

系统**必须**支持基于 Markdown + YAML frontmatter 的技能定义文件。P3 builtin 技能的运行时 SSOT 为 `apps/desktop/main/skills/packages/<packageId>/<version>/skills/<skillDir>/SKILL.md`；frontmatter 由真实 runtime loader 直接校验，正文仅作说明补充。

#### 接口契约

```typescript
/**
 * P3 技能清单——从 SKILL.md frontmatter 解析。
 * 注意：与 P1 的 SkillDefinition（运行时技能配置）是不同概念。
 * SkillManifest 描述技能文件的静态元数据，SkillDefinition 描述运行时管线配置。
 * 解析 SKILL.md 后，系统通过 SkillManifest 生成对应的 SkillDefinition 注册到 WritingOrchestrator。
 */
interface SkillManifest {
  /** 技能 ID（如 'consistency-check'） */
  id: string
  /** 技能显示名称 */
  name: string
  /** 技能描述 */
  ...
  /** 包 ID，必须与包目录一致 */
  packageId: string
  /** 输入类型，决定 selection/document 运行时校验 */
  inputType?: 'selection' | 'document'
  /** 输出类型（运行时可读取；当前由 UI/编排侧消费） */
  outputType?: 'replacement' | 'suggestion' | 'annotation' | 'new-content'
  /** 权限级别（运行时可读取；当前由 UI/编排侧消费） */
  permissionLevel?: PermissionLevel
  /** 真实 runtime 允许的上下文规则 */
  context_rules: {
    surrounding?: number
...
用于描述技能用途。真实运行时执行 prompt.system / prompt.user。
```
...
```
应用启动
  → 扫描 apps/desktop/main/skills/packages/<packageId>/<version>/skills/<skillDir>/SKILL.md
    → 解析 YAML frontmatter → SkillManifest
    → 通过 runtime validator 校验 version / packageId / context_rules / prompt
    → 合法技能进入真实 loader / executor 链
  → 用户触发技能（通过 Cmd+K 或技能面板）
    → 通过 `ai:skill:run` 进入真实 skill executor
    → 按 context_rules 组装上下文
    → 渲染 prompt.user（替换 {{input}}）
    → 调用 AI → 返回结果
```

#### Scenario: P3 加载 SKILL.md 技能定义

- **假设** `apps/desktop/main/skills/packages/pkg.creonow.builtin/1.0.0/skills/consistency-check/SKILL.md` 存在
- **当** 应用启动并扫描技能目录
- **则** 系统解析 frontmatter 并注册 `consistency-check` 技能
- **并且** 技能面板中出现「一致性检查」选项

---

### Requirement: P3 — 一致性检查技能（consistency-check）

系统**必须**提供一致性检查技能，检查当前文档或选中文本与项目角色/地点设定之间的矛盾。
...
```
用户触发一致性检查
  → WritingOrchestrator 接收 WritingRequest(skillId: 'consistency-check')
    → [1] 加载 SkillDefinition
    → [2] Context Engine 注入：当前文档内容 + 角色设定 + 地点设定
    → [3] 渲染 system prompt 模板
    → [4] 调用 AI（流式）
    → [5] AI 返回矛盾标注列表
    → [6] 渲染进程展示标注（不写回文档，仅提示）
```
...
#### Scenario: P3 一致性检查发现矛盾

- **假设** 角色「林远」设定为「28 岁，性格冷静」，当前章节中写到「林远暴躁地摔门」
- **当** 用户触发一致性检查
- **则** AI 返回标注：「位置："林远暴躁地摔门"，矛盾："林远设定性格冷静，但此处表现暴躁"，建议："改为'林远沉着脸，缓缓关上门'"」
- **并且** 结果以侧边栏标注形式展示，用户可逐条查看

#### Scenario: P3 一致性检查无矛盾

- **假设** 当前文档与所有角色/地点设定一致
- **当** 用户触发一致性检查
- **则** 返回 `{ passed: true, issues: [] }`
- **并且** Toast 通知「✅ 未发现一致性问题」

#### Scenario: P3 一致性检查在无设定时的降级

- **假设** 项目没有任何角色/地点设定
- **当** 用户触发一致性检查
- **则** 提示「请先添加角色/地点设定后再进行一致性检查」
- **并且** 不调用 AI

---

### Requirement: P3 — 对白生成技能（dialogue-gen）

系统**必须**提供对白生成技能，根据角色设定和场景上下文生成符合角色性格的对白。
...
| requiresSelection | `true`（选中对白位置或角色名） |
| requiresProjectContext | `true` |
...
```
用户选中文本 + 触发对白生成
  → WritingOrchestrator 接收 WritingRequest(skillId: 'dialogue-gen')
    → [1] 加载 SkillDefinition
    → [2] Context Engine 注入：选中文本上下文 + 角色设定 + 场景描写
    → [3] 渲染 system prompt（含角色性格/口头禅/关系）
    → [4] 调用 AI（流式）
    → [5] AI 返回对白候选
    → [6] Diff 预览 → 用户确认/拒绝 → 写回
```

#### Scenario: P3 为指定角色生成对白

- **假设** 用户选中文本「林远说：」，项目有角色设定「林远: 性格冷静，说话简练」
- **当** 用户触发对白生成
- **则** AI 根据角色设定生成符合林远性格的对白
- **并且** 生成的对白简练、语气冷静
- **并且** 用户可通过 Diff 预览确认或拒绝

#### Scenario: P3 对白生成考虑场景上下文

- **假设** 当前场景是紧张的追逐战，角色设定「林远: 前特种兵」
- **当** 用户触发对白生成
- **则** AI 生成的对白反映紧张氛围和角色专业背景
- **并且** 不生成与场景不符的轻松对白

#### Scenario: P3 对白生成无角色设定时

- **假设** 选中位置涉及的角色没有任何设定
- **当** 用户触发对白生成
- **则** AI 仅根据前文已有的信息推断角色语气
- **并且** 不编造角色未定义的口头禅或性格特征

---

### Requirement: P3 — 大纲展开技能（outline-expand）

系统**必须**提供大纲展开技能，将大纲要点展开为完整段落或章节内容。
...
```
用户选中大纲要点 + 触发大纲展开
  → WritingOrchestrator 接收 WritingRequest(skillId: 'outline-expand')
    → [1] 加载 SkillDefinition
    → [2] Context Engine 注入：选中大纲 + 前后章节上下文 + 角色/地点设定 + 风格偏好
    → [3] 渲染 system prompt（含项目风格/叙述人称/语气基调）
    → [4] 调用 AI（流式）
    → [5] AI 返回展开内容
    → [6] Diff 预览 → 用户确认 → 写回（追加在大纲后方或替换）
```

#### Scenario: P3 展开单个大纲要点

- **假设** 用户选中大纲文本「林远独自来到废弃仓库，发现了一封神秘信件」
- **当** 用户触发大纲展开
- **则** AI 根据项目风格设定（冷峻克制）和角色设定（林远: 退休刑警）展开为 2-3 段完整叙事
- **并且** 展开内容包含环境描写、角色心理和动作细节
- **并且** 用户通过 Diff 预览确认后写回

#### Scenario: P3 展开保持前后章节连贯

- **假设** 大纲要点位于第五章，第四章以林远乘车出发结尾
- **当** 用户触发大纲展开
- **则** 展开内容的开头与第四章结尾衔接自然
- **并且** 保持叙述视角和时间线连贯

#### Scenario: P3 大纲展开无前后上下文

- **假设** 大纲要点位于项目的第一个章节，前文为空
- **当** 用户触发大纲展开
- **则** AI 仅根据大纲内容和项目风格设定展开
- **并且** 功能正常，不报错

---

### P3 WritingEvent 扩展汇总

P3 阶段各模块新增的 WritingEvent 变体，统一汇总如下。P1 的 `WritingEvent` 联合类型在 P3 阶段应扩展为包含以下所有变体：

```typescript
/** P3 新增 WritingEvent 变体 */
type P3WritingEvent =
  // 3A Project Management
  | { type: 'project-config-updated'; timestamp: number; projectId: string; changedFields: string[] }
  // 3B Settings (Character)
  | { type: 'character-updated'; timestamp: number; projectId: string; characterId: string; action: 'created' | 'updated' | 'deleted' }
  // 3B Settings (Location)
...
完整的 P3 `WritingEvent` 联合类型为：`type WritingEvent = P2WritingEvent | P3WritingEvent`（P2WritingEvent 已包含 P1 全部变体）。各变体的详细定义分别见对应模块的 spec.md。

---

### P3 annotation 输出类型的管线处理

`outputType: 'annotation'` 的技能（如 `consistency-check`）不走标准的 `write-back` 管线步骤，改走 `annotation-render` 路径：
...
type PipelineStep =
  | /* ...P1 existing steps... */
  | 'annotation-render'  // annotation 类型技能的专用步骤，替代 write-back

/**
 * annotation 管线：
 * validate-input → assemble-context → select-model → call-ai
 *   → stream-response → annotation-render → post-hooks
 *
 * 与标准管线的区别：
 * 1. 跳过 permission-gate（annotation 不修改文档，无需确认）
 * 2. 跳过 write-back（annotation 不写回文档）
 * 3. 跳过 version-snapshot（无文档变更，无需版本快照）
 * 4. annotation-render 步骤解析 AI 输出为结构化结果
 */
```

#### annotation-render 步骤详细流程

```
AI 返回流式文本（结构化 JSON 或半结构化列表）
  → [1] AI 完成后（ai-done 事件），解析为 ConsistencyCheckResult
  → [2] 验证 JSON 结构完整性（schema 校验）
  → [3] 发射 'consistency-check-completed' WritingEvent
  → [4] 渲染进程接收事件，在侧边栏展示标注列表
  → [5] 不调用 write-back，不创建 version-snapshot
```

#### Scenario: P3 annotation 管线正常执行

- **假设** 用户触发一致性检查技能（`outputType: 'annotation'`）
- **当** AI 返回矛盾标注的 JSON 结果
- **则** `annotation-render` 步骤解析 JSON 为 `ConsistencyCheckResult`
- **并且** 发射 `consistency-check-completed` WritingEvent
- **并且** 不执行 `write-back` 和 `version-snapshot`
- **并且** 渲染进程在侧边栏展示标注列表
...
- 量化阈值：
  - SKILL.md 解析与注册 p95 < 50ms（单文件）
  - 一致性检查 AI 调用 p95 < 15s（依赖 AI 模型速度）
  - 对白生成/大纲展开首 chunk 返回 p95 < 3s
- 边界与类型安全：
  - SKILL.md frontmatter 必须通过 Zod schema 校验
  - 模板占位符 `{{}}` 必须在运行时校验存在性
- 失败处理策略：
  - SKILL.md 解析失败：跳过该技能，记录警告日志
  - AI 调用失败：沿用 P1/P2 的 WritingOrchestrator 错误处理
  - 上下文注入失败：降级为无项目上下文的技能执行

#### Scenario: P3 SKILL.md 格式错误时的降级

- **假设** 某 `SKILL.md` 文件 frontmatter 缺少必填字段 `id`
- **当** 应用启动扫描技能
- **则** 系统跳过该文件并记录警告 `SKILL_PARSE_FAILED: missing required field 'id'`
- **并且** 其他技能正常加载

---

### P3 不做清单（Skill Extension）

- ❌ 不做自定义技能创作/编辑界面（P3 仅内置技能）
- ❌ 不做技能市场/分享
- ❌ 不做 AI 自动选择技能（用户手动触发）
- ❌ 不做多技能串联/编排
- ❌ 不做技能的 A/B 测试框架
