...
interface SkillInputRequirement {
   ...
   * P3 技能通过 SkillManifest.contextRequirement.requiresProjectContext 声明，
...
系统**必须**支持基于 Markdown + YAML frontmatter 的技能定义文件。每个技能由一个 `.skill.md` 文件定义，文件头部 frontmatter 声明技能元数据，正文为 system prompt 模板。
...
interface SkillManifest {
  ...
  description: string
  /** 技能分类 */
  category: 'writing' | 'analysis' | 'generation'
  /** 技能作用域——P3 仅支持内置技能，`global`/`project` 作用域留待后续版本 */
  scope: 'builtin'
  /** 上下文要求（声明技能需要的上下文类型） */
  ...
  /** System prompt 模板（Markdown 正文部分） */
  systemPromptTemplate: string
}

/**
 * P3 技能上下文要求——声明技能需要的上下文类型。
 * 注意：与 P1 的 SkillInputRequirement（运行时输入校验）是不同概念。
 * SkillContextRequirement 描述技能对项目/文档上下文的依赖，
 * SkillInputRequirement 描述技能对用户输入（选区、指令）的校验要求。
 */
...
```
应用启动
  → 扫描 main/skills/packages/*.skill.md
    → 解析 YAML frontmatter → SkillManifest
    → 从 SkillManifest 生成 SkillDefinition 注册到 WritingOrchestrator 的技能注册表
  → 用户触发技能（通过 Cmd+K 或技能面板）
    → 意图路由匹配 SkillDefinition（由 SkillManifest 生成）
    → 组装上下文（按 contextRules 注入角色/地点/记忆）
    → 渲染 system prompt 模板（替换 {{}} 占位符）
    → 调用 AI → 流式返回结果
```

#### Scenario: P3 加载 SKILL.md 技能定义

- **假设** `main/skills/packages/` 目录下有 `consistency-check.skill.md`
- **当** 应用启动并扫描技能目录
