...
P3 阶段目标：从「单文档写作」升级为「项目制长篇创作」——用户可以创建包含多篇文档的创作项目，配置项目级风格/类型设定，AI 将在项目上下文中工作。

| 变更 | 描述 |
|------|------|
| P3 — 项目 CRUD 与多文档管理 | 完整的项目创建/读取/更新/删除，多文档组织，项目级设置 |
| P3 — 项目风格与类型配置 | 细化 genre/style/tone 等创作设定，注入 AI 上下文 |
| P3 — 项目切换的全量上下文重绑定 | 切换项目时联动 Settings（角色/地点）、Memory、Search 等 P3 新模块 |
...
| IPC 通道                   | 通信模式         | 方向            | 用途                |
| -------------------------- | ---------------- | --------------- | ------------------- |
| `project:project:create`           | Request-Response | Renderer → Main | 创建项目            |
| `project:project:createaiassist` | Request-Response | Renderer → Main | AI 辅助生成项目结构 |
...
- **则** 系统通过 `project:project:create` 创建项目
- **并且** 自动创建一个默认空白章节
- **并且** 工作台切换到新项目的编辑器视图
...
- **则** 系统通过 `project:project:createaiassist` 调用 LLM 生成项目结构
...
- `knowledgeGraphId`（string | null）：关联的知识图谱 ID

项目元数据的修改通过 IPC 通道 `project:config:update`（Request-Response）持久化。

#### Scenario: 用户编辑项目元数据

- **假设** 用户打开项目设置面板
- **当** 用户将叙述人称从「第一人称」改为「第三人称有限」，并设置目标字数为 200000
- **则** 系统通过 `project:config:update` 持久化变更
...
点击卡片打开对应项目（通过 `project:project:switch` 切换到编辑器视图）。

Dashboard 数据通过以下 IPC 通道获取：

| IPC 通道        | 通信模式         | 方向            | 用途             |
| --------------- | ---------------- | --------------- | ---------------- |
| `project:project:list`  | Request-Response | Renderer → Main | 获取所有项目列表 |
| `project:project:stats` | Request-Response | Renderer → Main | 获取项目统计数据 |
...
- **当** 用户点击「暗流」卡片
- **则** 系统通过 `project:project:switch` 切换到「暗流」项目
...
### Requirement: 项目切换必须触发 ProjectLifecycle 的解绑/绑定与资源清理

项目切换**必须**具备可验证的资源生命周期闭环，确保跨项目操作不会引入内存泄漏、文件句柄泄漏或残留后台任务。

- 切换流程必须遵循：`unbind ALL → 持久化切换 → bind ALL`。
- 解绑/绑定应有超时保护，单个服务失败不得导致系统静默卡死。
- ProjectLifecycle 仅负责“统一触发 unbind/bind 与顺序约束”；各模块的资源清理语义由对应模块 spec 定义并可回归验证：
  - IPC timeout -> abort：见 IPC spec（Scenario: BE-SLA-S2）
  - session 并发槽位回收：见 Skill System spec（Scenario: BE-SLA-S3）
  - project-scoped cache/watcher 清理：见 Context Engine spec（Scenario: BE-SLA-S4）

#### Scenario: BE-SLA-S1 项目切换先解绑再绑定且具备超时保护
...
### Requirement: projectLifecycle 项目切换必须串行化执行
...
#### Scenario: AUD-C1-S5 switchProject 期间重复切换到同一项目被幂等处理
...
### Requirement: 项目删除（两阶段：归档后永久清除）

项目**不可从 `active` 状态直接删除**；永久删除必须经历两个阶段：先归档（Archive），再永久清除（Purge）。这与下方"项目生命周期闭环"requirement 中的状态机约束一致：`active -> deleted` 为**禁止转换**。

**第一阶段：归档**通过 `project:lifecycle:archive` 将项目从 `active` 置为 `archived`，归档后项目只读、不可继续编辑。

**第二阶段：永久清除**仅当项目处于 `archived` 状态时，用户方可触发永久删除。清除确认流程：

1. 用户在已归档项目列表中触发「永久删除」
2. 系统弹出确认对话框，显示项目名称和包含的文档数量
3. 用户**必须**手动输入项目名称进行二次确认
4. 确认后系统通过 `project:project:delete` 永久删除项目及其所有关联数据（文档、知识图谱实体、角色/地点设定、简单记忆条目、FTS 索引、版本历史）

若对 `active` 项目直接调用 `project:project:delete`，系统**必须**拒绝并返回 `{ code: "PROJECT_DELETE_REQUIRES_ARCHIVE", message: "请先归档项目再删除" }`。

#### Scenario: 用户完成两阶段删除流程

- **假设** 项目「测试项目」处于 `active` 状态，包含 3 篇文档
- **当** 用户在 Dashboard 触发「归档项目」
- **则** 系统通过 `project:lifecycle:archive` 将状态更新为 `archived`
- **并且** 项目从默认列表移入「已归档」分组
- **当** 用户在已归档列表中触发「永久删除」
- **则** 系统弹出确认对话框，显示「永久删除项目"测试项目"将不可撤销地删除所有文档（3 篇）、角色/地点设定、记忆数据、搜索索引和版本历史。请输入项目名称确认删除。」
- **当** 用户输入「测试项目」并点击「确认永久删除」
- **则** 系统通过 `project:project:delete` 永久删除项目及所有关联数据
- **并且** 归档列表刷新，项目卡片消失
- **并且** Toast 通知「项目已永久删除」
...
- **当** 用户输入「测试」（不完整）
- **则** 「确认永久删除」按钮保持禁用状态（`disabled`）
- **并且** 用户无法执行永久删除

#### 说明：`project:project:delete` 与 `project:lifecycle:purge` 的关系

本模块同时公开两个与删除相关的 IPC 通道，二者**并存**，均属已发布 contract，**不得移除**：

| IPC 通道                    | 语义层                  | 使用场景                                                   |
| --------------------------- | ----------------------- | ---------------------------------------------------------- |
| `project:project:delete`    | 用户操作高层入口        | UI 侧用户二次确认后触发的永久删除动作（Dashboard / 项目设置） |
| `project:lifecycle:purge`   | 生命周期语义通道        | 生命周期状态机内对 `archived` 项目执行物理清除（`archived → deleted`） |

**两者共同遵守「归档优先」contract（archive-first）**：

- `active` 状态的项目**不能**直接通过任何一个通道删除，必须先经过 `project:lifecycle:archive` 进入 `archived` 状态
- `project:project:delete` 在执行删除前**必须**校验项目是否处于 `archived` 状态；若调用方传入 `active` 项目，则拒绝并返回 `PROJECT_DELETE_REQUIRES_ARCHIVE`
- `project:lifecycle:purge` 同理，仅接受 `archived` 项目作为操作目标

> **禁止**将 spec 读作「存在单一删除合同」或「`active` 项目可直接删除」——上述两种解读均与实际 contract 相悖。
...
- `archived -> deleted`：允许，执行物理删除（含文档、知识图谱、角色/地点设定、简单记忆、FTS 索引、版本）
...
- 量化阈值：
  ...
  - `project:config:update` p95 < 200ms
...
- **假设** 用户在两个中等规模项目间切换
- **当** 连续触发 30 次 `project:project:switch`
- **则** p95 耗时小于 1s
- **并且** 无一次请求返回未定义错误码
...
- `project:project:switch`：p50 < 400ms，p95 < 1s，p99 < 2s
- `project:project:create`：p50 < 200ms，p95 < 500ms，p99 < 900ms
...
### Requirement: P3 — 项目配置模型（ProjectConfig）

系统**必须**扩展项目元数据为完整的 `ProjectConfig`，支持创作项目的全局设定。
...
/** 项目风格设定——注入 Context Engine Rules 层 */
interface ProjectStyleConfig {
  /** 叙述人称 */
  narrativePerson: 'first' | 'third-limited' | 'third-omniscient'
  /** 体裁/类型标签（如 '都市悬疑'、'校园推理'） */
  genre: string
  /** 语言风格描述（自由文本，如 '简洁克制，短句为主'） */
...
```
ProjectConfig 变更
  → project:config:update IPC
    → 持久化到 SQLite
    → ProjectStyleConfig 同步到 Context Engine Rules 层
    → Memory System 的 project-scope 切换
    → Search 索引 scope 绑定
```
...
| IPC 通道 | 通信模式 | 方向 | 用途 |
|----------|---------|------|------|
| `project:config:get` | Request-Response | Renderer → Main | 获取项目完整配置 |
| `project:config:update` | Request-Response | Renderer → Main | 更新项目配置 |
| `project:style:get` | Request-Response | Renderer → Main | 获取风格设定（Context Engine 用） |

#### WritingEvent 扩展

```typescript
/** P3 新增 WritingEvent——项目配置变更 */
type ProjectConfigUpdatedEvent = {
...
| 错误场景 | code | 处理策略 |
|---------|------|---------|
| 项目不存在 | `PROJECT_NOT_FOUND` | 返回错误，引导回 Dashboard |
| 配置校验失败 | `PROJECT_CONFIG_INVALID` | 返回具体字段错误 |
| genre 为空 | `PROJECT_GENRE_REQUIRED` | P3 阶段 genre 为必填 |
| 项目数量超限 | `PROJECT_CAPACITY_EXCEEDED` | 返回错误，引导用户整理 |
| 删除未归档项目 | `PROJECT_DELETE_REQUIRES_ARCHIVE` | 拒绝操作，提示先归档 |
| 生命周期写入失败 | `PROJECT_LIFECYCLE_WRITE_FAILED` | 返回错误，保持原状态 |
| 项目名称冲突 | `PROJECT_NAME_CONFLICT` | 返回错误，提示重命名 |
...
- **假设** 用户在创建对话框中填写项目信息
- **当** 用户设置体裁为「都市悬疑」、语气为「冷峻克制」、叙述人称为「第三人称有限」
- **则** 系统通过 `project:project:create` 创建项目，`ProjectStyleConfig` 一并持久化
- **并且** `ProjectStyleConfig` 自动注入 Context Engine 的 Rules 层
- **并且** 后续 AI 续写的行文风格受此设定约束

#### Scenario: P3 修改风格设定后即时生效

- **假设** 用户在项目设置中将语气从「冷峻克制」改为「温暖幽默」
- **当** 用户保存设置
- **则** Context Engine 的 Rules 层中对应规则即时更新
- **并且** 下次 AI 续写将体现新的语气基调

---

### Requirement: P3 — 多文档管理

系统**必须**支持项目内的多文档组织，提供文档列表、排序、分组视图。
...
| IPC 通道 | 通信模式 | 方向 | 用途 |
|----------|---------|------|------|
| `project:documents:list` | Request-Response | Renderer → Main | 获取项目内文档列表 |
| `project:overview:get` | Request-Response | Renderer → Main | 获取项目统计概览 |

#### Scenario: P3 项目概览显示统计数据

- **假设** 项目「暗流」包含 12 个章节、3 个笔记、5 个角色、2 个地点
- **当** 用户在 Dashboard 查看项目卡片
- **则** 卡片显示总字数、文档数量、最后编辑时间
- **并且** 字数进度条显示相对于 `targetWordCount` 的完成度

#### Scenario: P3 按类型分组查看文档

- **假设** 用户切换文件树视图为「按类型分组」
- **当** 文件树刷新
- **则** 文档按「章节」「笔记」「设定文档」「角色卡」等分组展示
- **并且** 每组显示文档数量

---

### Requirement: P3 — 项目切换扩展绑定

P3 阶段项目切换**必须**扩展上下文切换范围，覆盖 Settings（角色/地点 CRUD）、Simple Memory、Full-text Search 模块。

切换序列扩展为：

1. 编辑器加载目标项目的当前文档
2. 文件树刷新为目标项目的文档列表
3. **Settings 模块**切换为目标项目的角色/地点列表（P3 新增）
4. **Simple Memory** 切换为目标项目的记忆记录（P3 新增）
5. **Search 索引**绑定为目标项目 scope（P3 新增）
6. 技能列表刷新（项目级技能变更）

#### Scenario: P3 项目切换联动角色/地点列表

- **假设** 用户从项目「暗流」切换到项目「星际迷航」
- **当** 项目切换完成
- **则** 角色列表从「暗流」的角色切换为「星际迷航」的角色
- **并且** 地点列表同步切换
- **并且** Memory 的 project-scope 重新绑定
...
- ❌ 不做项目级分支管理
- ❌ 不做跨项目文档引用
- ❌ 不做 AI 辅助创建的模板系统（保留 P1/P2 的手动 + AI 辅助两种方式）
