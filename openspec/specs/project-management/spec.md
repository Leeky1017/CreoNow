...
#### 说明：`project:delete` 与 `project:purge` 的关系

本模块同时公开两个与删除相关的 IPC 通道，二者**并存**，均属已发布 contract，**不得移除**：

| IPC 通道          | 语义层                  | 使用场景                                                   |
| ----------------- | ----------------------- | ---------------------------------------------------------- |
| `project:delete`  | 用户操作高层入口        | UI 侧用户二次确认后触发的永久删除动作（Dashboard / 项目设置） |
| `project:purge`   | 生命周期语义通道        | 生命周期状态机内对 `archived` 项目执行物理清除（`archived → deleted`） |

**两者共同遵守「归档优先」contract（archive-first）**：

- `active` 状态的项目**不能**直接通过任何一个通道删除，必须先经过 `project:archive` 进入 `archived` 状态
- `project:delete` 在执行删除前**必须**校验项目是否处于 `archived` 状态；若调用方传入 `active` 项目，则拒绝并返回 `PROJECT_DELETE_REQUIRES_ARCHIVE`
- `project:purge` 同理，仅接受 `archived` 项目作为操作目标
