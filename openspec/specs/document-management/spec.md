...
P3 阶段为文档管理新增 ProseMirror 格式的结构化导出——适配 ProseMirror 替换 TipTap 后的文档格式，支持 Markdown / Word / PDF 导出。

| 变更 | 描述 |
|------|------|
| P3 — ProseMirror 导出适配 | 从 ProseMirror JSON 导出 Markdown / DOCX / PDF |
| P3 — 项目级批量导出 | 按章节顺序合并导出整个项目 |
| P3 — 导出进度与错误处理 | 长文档导出进度反馈、格式能力矩阵 |
...
文档数据结构**必须**包含：`id`、`projectId`、`type`、`title`、`content`（ProseMirror JSON）、`status`、`sortOrder`、`parentId`（用于文件夹层级）、`createdAt`、`updatedAt`。
...
系统**必须**支持将文档导出为以下格式，并以 ProseMirror JSON 的结构语义作为 PDF / DOCX / Markdown 的真实输入源：

| 格式     | 扩展名  | 说明                                                                                                         | 能力级别（v0.1） |
| -------- | ------- | ------------------------------------------------------------------------------------------------------------ | ---------------- |
| Markdown | `.md`   | 结构化 Markdown 导出，保留标题层级、段落与换行、粗体、斜体、列表、链接、图片等语义                           | 结构化           |
| TXT      | `.txt`  | 纯文本格式                                                                                                   | 纯文本           |
| PDF      | `.pdf`  | 结构化 PDF 导出，至少保留标题层级、段落与换行、粗体、斜体、下划线、列表、引用、链接、行内代码、分隔线、图片  | 结构化           |
| DOCX     | `.docx` | 结构化 DOCX 导出，至少保留标题层级、段落与换行、粗体、斜体、下划线、列表、引用、链接、行内代码、分隔线、图片 | 结构化           |

若导出目标格式暂不支持某个编辑器已允许创建的节点或 mark，系统**必须**在写文件前显式报错并指出不支持项，**禁止**静默降级为纯文本导出。

导出操作通过以下 IPC 通道完成：

| IPC 通道                       | 通信模式         | 方向            | 用途         |
| ------------------------------ | ---------------- | --------------- | ------------ |
| `export:document:prosemirror`  | Request-Response | Renderer → Main | 导出单个文档 |
| `export:project:prosemirror`   | Request-Response | Renderer → Main | 导出整个项目 |

导出入口：文档右键菜单「导出」→ 选择格式 → 选择保存路径（通过 Electron 文件对话框）。

导出过程中**必须**显示进度指示（长文档可能耗时），完成后 Toast 通知成功。导出失败时**必须**显示错误原因；文档不存在时返回 `EXPORT_DOCUMENT_NOT_FOUND`。
...
- **则** 系统弹出文件保存对话框
- **当** 用户选择保存路径并确认
- **则** 系统通过 `export:document:prosemirror` 将 ProseMirror JSON 转换为 Markdown 并写入文件
- **并且** Toast 通知「导出成功：第一章.md」

#### Scenario: 用户将带格式的文档导出为 PDF 或 DOCX

- **假设** 当前文档包含标题、粗体、斜体、列表、链接与图片
- **当** 用户选择「导出 → PDF」或「导出 → DOCX」
- **则** 系统根据 ProseMirror JSON 的结构语义生成对应文件
- **并且** 导出结果不退化为纯文本平铺

#### Scenario: 命中暂不支持结构时导出前显式失败

- **假设** 当前文档包含某个编辑器允许创建、但目标导出格式尚未实现的结构节点或 mark
- **当** 用户执行导出
- **则** 系统在写文件前返回明确错误
- **并且** 错误中指出不支持的结构类型
- **并且** 不生成降级后的残缺文件
...
- 单文档正文大小上限：5 MB（ProseMirror JSON 序列化后）
...
- **当** 执行 `export:project:prosemirror`
...
#### Scenario: 导出目标路径不可写时返回写入错误

- **假设** 用户选择的保存路径没有写入权限或路径不可达
- **当** 执行导出
- **则** 系统返回 `{ code: "EXPORT_WRITE_ERROR", message: "无法写入目标路径，请检查权限" }`
- **并且** 不创建任何临时文件残留
...
- `export:document:prosemirror`：p50 < 500ms，p95 < 1.5s，p99 < 3s
...
> **阶段**: P3（项目制长篇创作）
> **设计决策**: P3 阶段编辑器已从 TipTap 迁移到原生 ProseMirror（P1 完成），导出管线必须适配 ProseMirror JSON 格式。

### Requirement: P3 — ProseMirror 文档导出

系统**必须**支持将 ProseMirror JSON 格式的文档导出为 Markdown / DOCX / PDF，保留结构语义。
...
interface ExportDocumentRequest {
  ...
  /** 输出路径（通过 Electron 文件对话框选择） */
  outputPath: string
  /** 可选；若提供则校验 documentId 属于该项目 */
  projectId?: string
}
...
#### ProseMirror → 目标格式的转换管线

```typescript
/** P3 ProseMirror 文档转换器 */
interface ProseMirrorExporter {
  /**
   * ProseMirror JSON → Markdown
   * 保留：标题层级、段落、粗体、斜体、列表、链接、图片、代码块、引用
   */
  toMarkdown(doc: ProseMirrorDocument): string

  /**
   * ProseMirror JSON → DOCX
   * 保留：标题层级、段落、粗体、斜体、下划线、列表、引用、链接、代码、分隔线、图片
   * 使用 docx 库生成
   */
  toDocx(doc: ProseMirrorDocument, options: ExportOptions): Buffer

  /**
   * ProseMirror JSON → PDF
   * 保留：标题层级、段落、粗体、斜体、下划线、列表、引用、链接、代码、分隔线、图片
   * 使用 pdf-lib 或类似库生成
   */
  toPdf(doc: ProseMirrorDocument, options: ExportOptions): Buffer
}
```

#### 格式能力矩阵

| ProseMirror 节点/标记 | Markdown | DOCX | PDF |
|----------------------|----------|------|-----|
| heading (h1-h6) | ✅ | ✅ | ✅ |
| paragraph | ✅ | ✅ | ✅ |
| bold | ✅ | ✅ | ✅ |
...
对于不在矩阵中的节点/标记，系统**必须**在导出前检测并报错，**禁止**静默降级为纯文本。
...
| 目标文档不存在 | `EXPORT_DOCUMENT_NOT_FOUND` | 阻断导出并提示文档不存在 |
...
#### Scenario: P3 将 ProseMirror 文档导出为 Markdown

- **假设** 文档「第一章」包含标题、段落、粗体和列表
- **当** 用户右击选择「导出 → Markdown」
- **则** 系统从 ProseMirror JSON 转换为 Markdown
- **并且** 保留标题层级（`# → ##`）、粗体（`**`）、列表（`- `）
- **并且** 导出文件可被其他 Markdown 编辑器正确渲染

#### Scenario: P3 将项目导出为 Word 文件
...
- **假设** 用户要将小说导出为 PDF 投稿给出版社
- **当** 用户选择「导出项目 → PDF」，设置 A4 页面、12pt 字体
- **则** 系统生成格式规范的 PDF
- **并且** 标题、章节号、段落间距、字体大小符合出版投稿要求

#### Scenario: P3 导出时遇到不支持的节点

- **假设** 文档包含自定义的 ProseMirror 节点（如 `mention`）
- **当** 用户导出为 PDF
- **则** 系统在导出前检测到 `mention` 节点不在格式能力矩阵中
- **并且** 返回错误 `{ code: "EXPORT_UNSUPPORTED_NODE", unsupported: ["mention"] }`
- **并且** 不生成残缺文件

#### Scenario: P3 大项目导出的进度反馈

- **假设** 项目有 50 个章节，合计 20 万字
- **当** 用户执行项目级 PDF 导出
- **则** 导出进度条实时更新：解析（0-30%）→ 转换（30-80%）→ 写入（80-100%）
- **并且** 显示当前处理的章节名
- **并且** UI 保持可交互
...
- 量化阈值：
  - 单文档 Markdown 导出 p95 < 500ms
  - 单文档 DOCX 导出 p95 < 2s
  - 单文档 PDF 导出 p95 < 3s
  - 项目级导出（50 章节）p95 < 30s
- 边界与类型安全：
  - `TypeScript strict` + zod
  - `export:*` 通道返回统一 `IPCResponse`
- 失败处理策略：
  - 导出失败必须清理临时文件（60s 内）
  - 不支持的节点必须显式报错
  - 禁止静默降级为纯文本
