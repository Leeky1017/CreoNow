...
| IPC 通道          | 通信模式         | 方向            | 用途         |
| ----------------- | ---------------- | --------------- | ------------ |
| `export:document:write` | Request-Response | Renderer → Main | 导出单个文档 |
| `export:project:write`  | Request-Response | Renderer → Main | 导出整个项目 |
...
- **则** 系统弹出文件保存对话框
- **当** 用户选择保存路径并确认
- **则** 系统通过 `export:document:write` 将 TipTap JSON 转换为 Markdown 并写入文件
...
- `export:document:write`：p50 < 500ms，p95 < 1.5s，p99 < 3s
...
| IPC 通道 | 通信模式 | 方向 | 用途 |
|----------|---------|------|------|
| `export:document:write` | Request-Response | Renderer → Main | 导出单文档（按 format 写出目标文件） |
| `export:project:write` | Request-Response | Renderer → Main | 导出项目（按 format 合并写出目标文件） |
