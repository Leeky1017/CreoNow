...
| 用户主动取消                     | `aborted`              | 立即终止并清理资源，不重试；不触发 onError 回调 |
| 非 OpenAI provider 路由到 bridge | `unsupported-provider` | IPC 层回退到 legacy 路径执行                    |
...
4. **错误分类扩展**：`retryable` / `non-retryable` / `partial-result` / `aborted` / `unsupported-provider`——前端据此决定展示"重试中"、"已失败"、"部分结果可查看"或静默取消；`unsupported-provider` 由主进程回退到 legacy 路径
...
And 如果 3 次重试均失败，错误升级为 partial-result（实现预留，P1 当前未启用）
  And 前端展示"部分结果可用"并允许用户查看已接收内容
```

#### Scenario: P1 用户在流式调用过程中取消
...
V1 阶段模型路由结果依赖 provider 解析，`selectProvider()` 为 async 调用；路由器本身不直接发起网络请求。
...
- ✅ 支持 primary/auxiliary 双模型路由——generation 类技能走 primary，辅助类技能（summarize/classify/extract 等）走 auxiliary；仅配置一个模型时两者共用
- ❌ 不做用户级模型偏好覆盖——V1 的 `preferredModel` 字段预留但不生效
- ❌ 不做 token 预估路由——V1 的 `estimatedTokens` 字段预留但不生效

#### Scenario: P1 WritingOrchestrator 通过路由器获取 provider
...
#### Scenario: P1 未配置 provider 时的启动校验
...
#### Scenario: P1 Judge 检测通过

- **假设** AI 续写内容符合所有约束和设定
- **当** Judge 评估完成
- **则** AI 面板输出底部显示绿色标签「质量校验通过」（`--color-success`）

#### Scenario: P1 Judge 服务不可用时的降级
...
#### Scenario: P1 主 provider 不可用时自动切换
...
#### Scenario: P1 全 provider 异常时进入可用降级
...
#### Scenario: P1 S4 identity 为空白时不产生占位分隔符
...
#### Scenario: P1 S4 getMessages 返回防御性拷贝
...
#### Scenario: P1 S2 Token 超预算裁剪最早历史
...
#### Scenario: P1 S4 预算不足以容纳全部历史时强制保留 system + current
