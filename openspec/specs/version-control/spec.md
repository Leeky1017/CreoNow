...
| `version:diff:transaction`   | Request-Response | Renderer → Main | 计算两段纯文本的差异并返回 ProseMirror 事务步骤 |
...
### Requirement: P2 — 纯文本差异事务计算

`version:diff:transaction` 接受两段纯文本（`before` 和 `after`），计算差异并返回 ProseMirror 可应用的事务步骤（`TransactionSpec`）。用于 AI 续写/改写后将差异应用到编辑器文档。

#### IPC 通道

| IPC 通道 | 通信模式 | 方向 | 用途 |
|----------|---------|------|------|
| `version:diff:transaction` | Request-Response | Renderer → Main | 计算两段纯文本差异并返回事务步骤 |

**请求**：`{ before: string, after: string }`

**响应**：`{ steps: Array<{ type: "insert" | "delete" | "replace", from: number, to: number, text?: string }>, before: string, after: string, stats: { insertions: number, deletions: number, replacements: number, totalChanges: number, insertedChars: number, deletedChars: number } }`

#### Scenario: AI 改写后计算差异事务

- **假设** AI 对一段文本进行了改写
- **当** 前端调用 `version:diff:transaction`，传入改写前后的纯文本
- **则** 返回包含 `steps`、`stats` 的事务描述
- **并且** `steps` 中每个步骤包含 `type`（insert/delete/replace）、`from`、`to` 和可选的 `text`
