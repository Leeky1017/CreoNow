...
| `version:snapshot:list`   | Request-Response | Renderer → Main | 列出文档的版本历史 |
| `version:snapshot:read`   | Request-Response | Renderer → Main | 读取某个版本内容   |
| `version:diff`            | Request-Response | Renderer → Main | 计算两个版本差异   |
| `version:rollback`        | Request-Response | Renderer → Main | 恢复历史版本       |
...
- **当** 它接入版本管理模块
- **则** 只依赖 `version:snapshot:*`、`version:diff`、`version:rollback`
...
- 边界与类型安全：
  ...
  - 当前公开的 `version:snapshot:*` / `version:diff` / `version:rollback` 通道必须由 zod 校验
