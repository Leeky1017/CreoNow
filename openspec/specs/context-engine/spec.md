...
| 变更 | 描述 |
|------|------|
| P1 — 中文 Token 估算 | CJK/Emoji 字形簇 ≈ 1.5 tokens/cluster，非 CJK ≈ 0.25 tokens/byte |
...
| Emoji 字形簇（含 ZWJ、区域旗帜、keycap） | 1.5 tokens/grapheme cluster | P1 验收口径：emoji 按 CJK 处理，避免多码点 emoji 在预算路径出现不一致 |
...
```typescript
/**
 * CJK/Emoji 感知的 token 估算。
 * CJK 字符与 emoji 字形簇按 1.5 tokens 计，非 CJK 按 0.25 tokens/byte。
 */
function estimateTokens(text: string): number {
  let rawTokens = 0;
  for (const grapheme of segmentByGrapheme(text)) {
    if (isCjk(grapheme) || isEmojiGrapheme(grapheme)) {
      rawTokens += 1.5;
    } else {
      rawTokens += new TextEncoder().encode(grapheme).length / 4;
    }
  }
...
#### Scenario: emoji 视为 CJK 类字形簇

- **假设** 输入为 `👩‍💻`（单个 emoji 字形簇，含 ZWJ）
- **当** 调用 `estimateTokens(text)`
- **则** emoji 贡献 `1.5`，返回 `Math.ceil(1.5)` = 2
- **并且** 预算/裁剪链路与 CJK 计数语义保持一致
