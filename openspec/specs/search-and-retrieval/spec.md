...
P3 阶段将搜索降级为**纯全文检索**——仅保留 SQLite FTS5。这是有意的范围控制：「先让用户能找到内容，再谈语义理解」。

| 变更                | 描述                                   |
| ------------------- | -------------------------------------- |
| P3 — 项目级全文搜索 | 项目 scope 的 FTS5 全文检索 + 跳转定位 |

**P3 明确降级**：P3 只做 FTS。检索路径仅限 KG + FTS5。

## Purpose

本规格覆盖 Search & Retrieval 模块；**当前 P3 可验收合同仅限项目级全文检索（FTS）**。

### Scope

| Layer    | Path                                                                                  |
| -------- | ------------------------------------------------------------------------------------- |
| Backend  | `main/src/services/search/`                                                           |
| IPC      | `main/src/ipc/search.ts`                                                              |
...
全文检索基于 SQLite FTS5 扩展实现，索引内容为文档的纯文本内容（ProseMirror JSON → plain text）。
...
| `search:fts:indexstatus` | Request-Response | Renderer → Main | 查询索引就绪状态 |
...
2. 去重：按 `documentId + chunkId` 去重
3. 排序打分：

```
finalScore = 0.80 * bm25Norm + 0.20 * recencyScore
```

4. 结果截断：返回 Top 50，超过部分分页加载
...
| 类别         | 最低覆盖要求                                    |
| ------------ | ----------------------------------------------- |
| 网络/IO 失败 | 索引文件写入失败、索引重建失败                  |
| 数据异常     | 文档编码损坏、chunk offset 非法                 |
...
| `search:fts:indexstatus` | Request-Response | Renderer → Main | 查询当前项目索引状态 |
