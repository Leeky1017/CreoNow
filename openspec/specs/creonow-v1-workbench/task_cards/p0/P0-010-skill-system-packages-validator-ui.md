...
- [x] skill package：
  - [x] `SKILL.md` 使用 YAML frontmatter + markdown body
  - [x] 至少提供 1 个 builtin skill（推荐 `builtin:polish`）
- [x] validator：
  - [x] 缺字段/类型错误 → `INVALID_ARGUMENT`，且 `details` 包含 fieldName
  - [x] `context_rules` 未知字段必须拒绝（硬禁）
- [x] scopes：
  - [x] builtin/global/project 三种 scope 的目录落点写死并实现
  - [x] 同 id 冲突的覆盖规则写死（project > global > builtin），且最终只保留单事实源
- [x] IPC：
  - [x] `skill:list` 返回 `enabled/valid/error_*`
  - [x] `skill:toggle` 生效且可持久化
- [x] UI：
  - [x] AI Panel skills popup 可打开并列出 skills
  - [x] 选择 skill 后可触发 `ai:skill:run`
  - [x] 命令面板 `Cmd/Ctrl+P` 可打开（最小可用）
- [x] 禁用 skill：
  - [x] UI 中该 skill 不可运行（禁用态或隐藏，但行为必须写死且可测）

## Tests

- [x] Unit：`skillValidator.test.ts`
  - [x] 未知 context_rules 字段拒绝
  - [x] YAML 语法错误 → invalid + 可读 error_message
- [x] E2E（Windows）`skills.spec.ts`
  - [x] `skill:list` 返回至少 1 个 enabled+valid skill
  - [x] 禁用该 skill → UI 不可运行或运行返回明确错误码（断言）
...
- Issue: #38
- PR: #44
- RUN_LOG: `openspec/_ops/task_runs/ISSUE-38.md`
