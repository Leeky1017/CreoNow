...
- [ ] `ai:skill:run`：
  - [ ] 返回 `runId`
  - [ ] `stream=false` 返回完整结果（或 best-effort）
  - [ ] `stream=true` 通过 `ai:skill:stream` 推送 delta
- [ ] `ai:skill:cancel`：
  - [ ] 幂等（重复 cancel 返回 ok）
  - [ ] cancel 后不再有 delta 推送
  - [ ] 返回/事件体现 `CANCELED`
- [ ] timeout：
  - [ ] 超时返回 `TIMEOUT`
  - [ ] 超时后不再推送 delta
- [ ] upstream error：
  - [ ] provider/proxy 5xx 映射为 `UPSTREAM_ERROR`
- [ ] Fake-first：
  - [ ] Windows CI 默认走 Fake AI（无需真实 key）
  - [ ] Fake server 支持四模式：success/delay/timeout/upstream-error（触发方式写死并可断言）
  - [ ] success 输出包含固定片段（例如 `E2E_RESULT`）用于断言

## Tests

- [ ] E2E（Windows）`ai-runtime.spec.ts`
  - [ ] success：运行 builtin skill → 断言输出包含 `E2E_RESULT`
  - [ ] delay：断言 UI 进入 loading 状态，最终完成
  - [ ] timeout：断言 UI 显示 timeout 错误且错误码为 `TIMEOUT`
  - [ ] upstream-error：断言错误码为 `UPSTREAM_ERROR`
  - [ ] cancel：启动 stream → 立刻 cancel → 断言最终状态为 canceled 且不再增长输出
