...
P3 阶段将三层记忆架构降级为**简单记忆系统 + L1 会话记忆**：

| 变更 | 描述 |
|------|------|
| P3 — 简单记忆（Simple Memory） | 用户偏好 key-value 存储 + 项目级角色/地点设定自动注入 AI |
| P3 — L1 会话记忆（Session Memory） | 每会话持久化上下文注入：风格 / 参考 / 偏好 / 笔记，带时间衰减 + FTS5 关键词加权 + 15% 预算上限 |
| P3 — Forked Agent 注入 | 参考 CC memdir/ 模式，precision > recall |

**P3 明确降级**：三层记忆架构的情景记忆（Episodic）/ 语义记忆（Semantic）、记忆蒸馏、溯源、冲突检测均推迟到 P4+。P3 实现 key-value 偏好 + 角色设定注入 + L1 会话记忆。

## Purpose

本规格覆盖 Memory System 模块的长期路线图；**当前 P3 可验收合同包括简单记忆系统（key-value 偏好存储 + 项目级角色/地点设定自动注入 AI）和 L1 会话记忆（per-session context injection with time-decay scoring）**。三层记忆架构的高阶能力（情景记忆 / 语义记忆 / 蒸馏 / 溯源 / 冲突检测）仍保留为 P4+ 规划。
