...
本规格覆盖 Memory System 模块的长期路线图；**当前 P3 可验收合同仅限简单记忆系统（key-value 偏好存储 + 项目级角色/地点设定自动注入 AI）**。三层记忆架构（工作记忆 / 情景记忆 / 语义记忆）、记忆衰减、蒸馏、溯源、冲突检测仍保留为 P4+ 规划，不构成 P3 的必需实现或验收门槛。
...
### Requirement: 三层记忆架构（P4+ 规划，非 P3 验收）

在 P4+ 阶段，系统**必须**实现三层记忆架构，每层有独立的存储机制、生命周期和用途。当前 P3 不要求实现该能力：
...
**Layer 1：会话记忆（Session Memory）** _[P3-04 已实现]_

- 会话感知的上下文注入层，**持久化到 SQLite**（`session_memory` 表 + `session_memory_fts` FTS5 虚拟表）
- 存储内容：风格偏好（style）、参考信息（reference）、用户偏好（preference）、笔记（note）
- 存储方式：SQLite（better-sqlite3），CRUD 通过 `services/memory/sessionMemory.ts` `createSessionMemoryService()`
- 检索：FTS5 关键词匹配 + 按 session_id/project_id 范围过滤（INV-4 合规：无额外向量存储）
- 排序策略：指数时间衰减（半衰期 24 小时），base_score × 0.5^(age / half_life)
- 注入方式：通过 `sessionMemoryFetcher` 注入 Context Engine 的 **Settings 层**
- 预算：硬性上限 = 总 context budget 的 15%（通过 `injectForContext` 中 `L1_BUDGET_CAP_RATIO` 实施）
- 过期机制：每条记忆可设 `expiresAt`，过期条目在读取时过滤 + `purgeExpired()` 批量清理

**Layer 2：情景记忆（Episodic Memory）**

- 具体的、可回溯的交互事件记录，是语义记忆蒸馏的原材料
- 持久化到 SQLite，每个 Episode 一行
- 索引维度：时间索引（timestamp）、项目索引（projectId）、场景类型索引（sceneType）、语义索引（embedding 向量，依赖 P4+ embedding 服务恢复）
- 召回方式：场景类型 + 语义相似度混合召回
...
- **假设** 用户请求续写一段打斗场景
- **当** 系统组装 AI 上下文
- **则** 会话记忆通过 FTS5 关键词匹配提供相关会话上下文和风格偏好
- **并且** 情景记忆通过语义检索召回过去 3 次用户在打斗场景中的交互记录
- **并且** 语义记忆注入规则「动作场景偏好短句、快节奏」
- **并且** 三层记忆经 Context Engine 整合后喂入 AI

#### Scenario: 会话记忆注入超出预算上限

- **假设** 当前会话有大量记忆条目，总 token 超过 context budget 的 15%
- **当** 系统通过 `injectForContext` 组装 L1 注入内容
- **则** 按时间衰减排序后逐条累加，超出 15% 上限时截断
- **并且** 返回 `truncated: true` 标记
- **并且** 截断不影响其他 context 层的正常注入

#### Scenario: 过期会话记忆清理

- **假设** 某些会话记忆条目已过 `expiresAt` 时间
- **当** 系统调用 `purgeExpired()` 或在列表查询中过滤
- **则** 过期条目被排除在 FTS5 搜索结果和注入内容之外
- **并且** `purgeExpired()` 从数据库物理删除过期条目
- **并且** 未过期和无过期时间的条目不受影响

---

### Requirement: 情景记忆记录与隐式反馈提取（P4+ 规划，非 P3 验收）

在 P4+ 阶段，系统**必须**在每次 AI 技能执行后自动记录情景记忆（Episode），捕获用户的显式和隐式反馈信号。当前 P3 不要求实现该能力。
...
### Requirement: episodicMemoryService 并发状态访问必须通过 per-project 互斥锁保护（P4+ 规划，非 P3 验收）

在 P4+ 阶段，`episodicMemoryService.ts` 内部维护 `distillingProjects`、`scheduledBatchDistillProjects`、`pendingEpisodeCountByProject` 等共享可变 Map/Set，并在 `recordEpisode` 持久化与 `distillSemanticMemory`（手动触发）读取项目级 episode 快照时存在并发竞争。所有对这些共享状态的读写与“写入 episode / 触发蒸馏 / 读取快照”决策**必须**在 per-project 互斥锁保护下执行，防止并发异步操作导致丢失更新或蒸馏遗漏。当前 P3 不要求实现该能力。

#### Scenario: AUD-C1-S1 并发 recordEpisode 不丢失更新
...
#### Scenario: AUD-C1-S2 recordEpisode 与 distillSemanticMemory（manual）互斥
...
#### Scenario: AUD-C1-S3 不同项目的操作互不阻塞
...
### Requirement: 语义记忆蒸馏（P4+ 规划，非 P3 验收）

在 P4+ 阶段，系统**必须**从情景记忆中自动蒸馏出抽象的偏好规则，生成语义记忆条目。当前 P3 不要求实现该能力。
...
### Requirement: 记忆衰减与生命周期管理（P4+ 规划，非 P3 验收）

在 P4+ 阶段，系统**必须**实现基于遗忘曲线的时间衰减模型，管理记忆的生命周期。当前 P3 不要求实现该能力。
...
| 记忆类型         | 预算              | 超出处理         |
| ---------------- | ----------------- | ---------------- |
| 会话记忆         | 15% context budget（SQLite） | 按时间衰减排序截断 |
...
- 实时：会话记忆过期过滤（每次 list/inject 时检查）+ purgeExpired 批量清理
- 每日：情景记忆衰减分数重算（应用启动时）
- 每周：批量压缩 + 语义记忆蒸馏
- 每月：淘汰已压缩的过期情景记忆
...
### Requirement: 记忆冲突检测与解决（P4+ 规划，非 P3 验收）

在 P4+ 阶段，系统**必须**检测语义记忆规则之间的冲突，并按策略自动或提示用户解决。当前 P3 不要求实现该能力。
...
### Requirement: 记忆面板（Memory Panel）——用户可见性与控制（P4+ 规划，非 P3 验收）

在 P4+ 阶段，系统**必须**提供记忆面板作为用户查看和管理 AI 记忆的统一界面，位于左侧栏（可通过 Icon Bar 扩展入口访问或在设置中打开）。
...
### Requirement: 记忆溯源（Memory Provenance）（P4+ 规划，非 P3 验收）

在 P4+ 阶段，系统**必须**支持记忆溯源——用户可查看 AI 输出是基于哪些记忆做出的决策。
...
### Requirement: 存储策略与淘汰机制执行细则（LRU + TTL）（P4+ 规划，非 P3 验收）

在 P4+ 阶段，记忆存储**必须**采用可验证的分层淘汰策略，避免无限增长。当前 P3 不要求实现该能力。
