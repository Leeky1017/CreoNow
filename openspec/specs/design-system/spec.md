...
| Layer  | Path                                                                           | 状态 |
| ------ | ------------------------------------------------------------------------------ | ---- |
| 设计源 | `design/system/01-tokens.css`                                                  | <!-- planned --> planned |
...
1. **源文件层**（`design/system/01-tokens.css` <!-- planned -->）：Token 的权威定义，由设计决策驱动。
...
`renderer/src/styles/tokens.css` 与 `design/system/01-tokens.css`（均为 <!-- planned --> planned）中的 Token 变量名和值**必须**保持一致。新增、修改、删除 Token 时，两个文件必须同步更新。
