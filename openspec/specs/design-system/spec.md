...
| Layer  | Path                                                                           | 状态 |
| ------ | ------------------------------------------------------------------------------ | ---- |
| 设计源 | `design/system/01-tokens.css`                                                  | <!-- active --> active |
| 运行时 | `apps/desktop/renderer/src/styles/tokens.css`                                  | <!-- planned --> planned |
| 主题桥 | `apps/desktop/renderer/src/styles/main.css`（`@theme` 块）                     | <!-- planned --> planned |
| 组件层 | `apps/desktop/renderer/src/components/primitives/`                             | <!-- planned --> planned |
| 参考   | `docs/CN-设计文稿.md`、`docs/cn-frontend-ssot/10-architecture/`               | 已存在 |
...
1. **源文件层**（`design/system/01-tokens.css` <!-- active -->）：Token 的权威定义，由设计决策驱动。
2. **运行时层**（`renderer/src/styles/tokens.css` <!-- planned -->）：与源文件层保持完全同步，供 Electron 渲染进程使用。
3. **Tailwind 桥接层**（`renderer/src/styles/main.css` <!-- planned --> 的 `@theme` 块）：将 Token 映射到 Tailwind v4 namespace，使 utility class 可用。
...
`renderer/src/styles/tokens.css`（<!-- planned --> planned）与 `design/system/01-tokens.css`（<!-- active --> active）中的 Token 变量名和值**必须**保持一致。`renderer/src/styles/tokens.css` 尚未创建；一旦落地，同步契约立即生效。新增、修改、删除 Token 时，两个文件必须同步更新。
