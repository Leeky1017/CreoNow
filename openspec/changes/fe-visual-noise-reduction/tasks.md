...
- [ ] 1.1 审阅并确认需求边界：逐区域审计并移除非功能性边框，以间距与排版为主分组手段。分隔线统一使用 `--color-separator`。不改功能逻辑。
- [ ] 1.2 审阅并确认错误路径与边界路径：交互卡片（hover/focus 反馈）保留轻边框；纯装饰性边框移除。
- [ ] 1.3 审阅并确认验收阈值与不可变契约：默认态不得出现多层 border 包裹；分隔线必须使用 `--color-separator` Token（不使用 `--color-border-default`）。
- [ ] 1.4 依赖同步检查（Dependency Sync Check）：建议先行 `fe-rightpanel-ai-tabbar-layout`、`fe-rightpanel-ai-guidance-and-style`
...
- [ ] 3.1 `WB-FE-VIS-S1`：读取 AiPanel.tsx 源码，统计非交互 border class 数量，断言不超过阈值。
  - 期望红灯原因：当前 ai/ 目录 55 处 border，大量为非功能性装饰。
- [ ] 3.2 `WB-FE-VIS-S2`：读取 DashboardPage.tsx，匹配项目卡片区域，断言默认态无 border class。
  - 期望红灯原因：当前项目卡片默认态有 border。
- [ ] 3.3 `WB-FE-VIS-S3`：扫描 ai/dashboard/settings 下 `.tsx`，找出分隔用途的 `--color-border-default`，断言为 0。
  - 期望红灯原因：当前分隔线使用 `--color-border-default` 而非 `--color-separator`。
- 运行：`pnpm -C apps/desktop test:run features/__tests__/visual-noise-guard`

## 4. Green（最小实现通过）

- [ ] 4.1 `features/ai/`：移除请求区/错误卡片/候选卡片的非功能性 border，保留交互卡片 hover border → S1 转绿
- [ ] 4.2 `features/dashboard/DashboardPage.tsx`：项目卡片默认态移除 border，hover 态改为 `hover:border-[var(--color-separator)]` → S2 转绿
- [ ] 4.3 `features/settings-dialog/`：section border → 间距分组 + `--color-separator` 分隔线
- [ ] 4.4 全局替换分隔用途的 `--color-border-default` → `--color-separator` → S3 转绿

## 5. Refactor（保持绿灯）

- [ ] 5.1 确认移除 border 后间距足够表达层级（视觉回归检查）
- [ ] 5.2 若 PanelContainer Composite 已就绪，将 panel 分隔样式收敛到 Composite 层
...
- [ ] 6.4 记录 Dependency Sync Check：确认上游 change 状态
