...
- [ ] 确认项目文件是 JSON/Markdown 明文（无加密封装）
- [ ] 确认存储目录由用户选择（非系统级保护目录）
- [ ] 记录关键代码路径和文件行号
...
- [ ] 搜索 `safeStorage` / `SecretStorageAdapter` 的实现
  - 命令：`grep -rn "safeStorage\|SecretStorageAdapter\|safeStorage\.encrypt\|safeStorage\.decrypt" apps/desktop/main/src/`
- [ ] 确认 API Key 确实通过 `safeStorage` 加密存储
- [ ] 确认加密范围仅限 API Key 等凭证，创作内容不在加密范围内
- [ ] 记录 `SecretStorageAdapter` 具体实现的文件路径和行号
...
- [ ] 核对 `06` §五存储格式总表的每一行与代码现场是否一致
- [ ] 核对 `08` §五-§六中 KG / Memory / SecretStorage 的结论
- [ ] 记录任何审计报告与代码现场不一致的条目
...
- [ ] 创建 `docs/release/v0.1-data-safety-boundary.md`
- [ ] 写入七个章节标题：声明目标、数据存储总表、三个明确"不"、安全边界依赖、已实现的安全措施、方向性演进计划、用户建议
- [ ] 在声明目标章节填入文档目的说明
...
- [ ] **不加密创作内容**：创作内容（手稿、KG、Memory）以明文形式存储在本地文件系统，无应用级加密
- [ ] **不远程同步**：所有数据仅存在于本地，不传输到任何远程服务器（API 调用除外）
- [ ] **不提供应用级备份**：数据持久性依赖用户自行文件备份。引用 A0-08 结论（如已产出）或 `06` §4.1 审计结论

### Task 2.4: 编写安全边界依赖与已实现措施

- [ ] 安全边界依赖：安全性取决于 OS 级保护——Windows BitLocker 磁盘加密、NTFS 文件权限、用户账户隔离
- [ ] 已实现的安全措施：`safeStorage` 加密 API Key，附代码路径引用

### Task 2.5: 编写演进计划与用户建议

- [ ] 方向性演进计划：v0.2 评估 SQLCipher / 加密 FS 对 KG + Memory 的可行性；v0.3 用户可选工作区加密——仅方向标注，不承诺版本
- [ ] 用户建议：启用 BitLocker、定期手动备份项目目录、避免在共享账户电脑上存储敏感手稿
...
- [ ] 逐数据类型核对：加密状态标记是否与代码证据一致
- [ ] 确认 API Key 以外的数据类型无"已加密"误标
- [ ] 确认三个"不"是否与代码现场一致（搜索是否有远程同步 / 加密 / 备份的真实实现）
...
- [ ] 模拟 A0-06 执行 Agent 视角：凭本文档可直接填写事实表「数据安全边界」章节
- [ ] 确认数据存储总表格式与事实表结构兼容
- [ ] 确认三个"不"可直接用作事实表的安全边界声明文本
...
| 验收标准 | 对应任务 | 验证方式 | 状态 |
|----------|---------|---------|------|
| AC-1: 文档存在且含七章节 | Task 2.1 | 文件存在 + 结构审查 | [ ] |
| AC-2: 五种数据类型覆盖 | Task 1.1-1.4, 2.2 | 文档审查 | [ ] |
| AC-3: 每类型含四列 | Task 2.2 | 表格完整性 | [ ] |
| AC-4: API Key 标 🔐 并引证 | Task 1.3, 2.2 | 证据链审查 | [ ] |
| AC-5: 创作内容标 📄 | Task 1.1-1.2, 2.2 | 标记一致性 | [ ] |
| AC-6: 三个"不"完整 | Task 2.3 | 文档审查 | [ ] |
