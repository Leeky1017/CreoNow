...
- [ ] 1.1 审阅并确认 C10 边界：匹配引擎替换 mock recognizer。
- [ ] 1.2 审阅并确认输入来源：实体名 + aliases。
- [ ] 1.3 审阅并确认性能与边界阈值：100×1000<10ms、重叠名称/中文/空输入。
...
| KG-S2-EM-S2 | `apps/desktop/main/src/services/kg/__tests__/entityMatcher.performance.test.ts` | `matches 100 entities in 1000 chars under 10ms` | 100×1000 性能阈值可验证        |
...
- [ ] 3.1 先写基础匹配失败测试并确认 Red。
- [ ] 3.2 先写性能阈值失败测试并确认 Red。
- [ ] 3.3 先写边界场景失败测试并确认 Red。

## 4. Green（最小实现通过）

- [ ] 4.1 仅实现使 Red 转绿的最小匹配逻辑。
- [ ] 4.2 逐条使失败测试转绿，不引入无关能力。
- [ ] 4.3 复跑映射测试，确认 Scenario 对应测试全部通过。

## 5. Refactor（保持绿灯）

- [ ] 5.1 对热点路径做最小重构，避免过度抽象。
- [ ] 5.2 清理临时实现并保持测试全绿。
- [ ] 5.3 回归执行后保持全绿。
