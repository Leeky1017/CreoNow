...
- [ ] 测试：扫描全部 `.tsx` 文件中的 JSX 表达式，断言无 `{error.code}` / `{error.message}` / `{state.error.code}` / `{state.error.message}` / `{lastError.code}` / `{lastError.message}` 模式
- [ ] 测试：扫描全部 `.tsx` / `.ts` 文件中的模板字符串，断言无 `` `${...error.code}` `` / `` `${...error.message}` `` 拼接模式
- [ ] 测试：扫描全部 `.tsx` 文件，断言无 `"ACTION_FAILED:` / `"NO_PROJECT:` 硬编码字符串
...
- [ ] 测试：渲染 `ExportDialog`，模拟导出失败返回 `{ code: "DB_ERROR", message: "SQLITE_CONSTRAINT: UNIQUE constraint failed" }`，断言 DOM 中不包含 `DB_ERROR` 和 `SQLITE_CONSTRAINT`
- [ ] 测试：同上场景，断言 DOM 中包含 `getHumanErrorMessage()` 对 `DB_ERROR` 的返回值
- [ ] 测试：断言错误文本元素使用了 `var(--color-text-error)` Token（通过 className 或 computed style）
- [ ] 测试：断言错误区域存在 `role="alert"` 属性
...
- [ ] 测试：渲染 `QualityPanel`，模拟 Judge 错误 `{ code: "MODEL_NOT_READY", message: "Judge model ensure is not implemented (non-E2E build)" }`，断言 DOM 中不包含 `MODEL_NOT_READY` 和 `non-E2E build`
- [ ] 测试：渲染 `QualityPanel`，模拟 Constraints 错误，断言 DOM 中包含人话文案而非技术码
- [ ] 测试：断言错误区域存在 `role="alert"` 属性
...
- [ ] 测试：渲染 `JudgeSection`，模拟检测失败返回错误，断言 DOM 中不包含 `error.code` 值
- [ ] 测试：渲染 `AiSettingsSection`，模拟 API Key 验证失败，断言 DOM 中包含人话文案
- [ ] 测试：渲染 `DashboardPage`，模拟操作失败，断言 DOM 中不包含模板字符串拼接的技术信息
...
- [ ] 测试：触发 CommandPalette 中 Settings 不可用场景，断言不展示 `"ACTION_FAILED: Settings dialog not available"`
- [ ] 测试：触发 CommandPalette 中 No Project 场景，断言不展示 `"NO_PROJECT: Please open a project first"`
- [ ] 测试：断言展示的文案为 `t()` 返回值（通过 mock i18n 验证）
- [ ] 测试：切换 locale 后，错误文案跟随切换
...
- [ ] 测试：渲染 `AiErrorCard` 并设置 `service_error` 状态和 `errorCode: "AI_RATE_LIMITED"`，断言 DOM 中不包含 `AI_RATE_LIMITED`
- [ ] 测试：断言 DOM 中包含 `getHumanErrorMessage()` 对 `AI_RATE_LIMITED` 的返回值
...
- [ ] 测试：在中文 locale 下渲染一个收口组件（如 `ExportDialog`）的错误状态，记录错误文案
- [ ] 测试：切换到英文 locale，断言错误文案变更为英文翻译
- [ ] 测试：两次渲染的错误文案不相等
...
- [ ] 测试：`zh-CN.json` 中包含 `commandPalette.error.*` 下全部错误场景的 key（至少覆盖 settings、export、layout、noProject）
- [ ] 测试：`en.json` 中包含相同结构的 key，数量与 `zh-CN.json` 一致
- [ ] 测试：所有 `commandPalette.error.*` 值不包含 `ACTION_FAILED` 或 `NO_PROJECT` 等硬编码前缀
...
- [ ] `ExportDialog.tsx` L425, L427：导入 `getHumanErrorMessage`，替换错误渲染
- [ ] `QualityPanel.tsx` L112-114, L197-199：替换 Judge 和 Constraints 两处错误渲染
- [ ] `VersionPreviewDialog.tsx` L79：替换错误渲染
- [ ] 确认三个组件的错误文本元素使用 `var(--color-text-error)` Token
- [ ] 确认三个组件的错误区域设置 `role="alert"`
...
- [ ] `InfoPanel.tsx` L151-152：替换错误渲染
- [ ] `MemoryPanel.tsx` L498, L510：替换 `{state.error.code}` + `{state.error.message}` 渲染
- [ ] `AnalyticsPage.tsx` L151：替换错误渲染
- [ ] 确认三个组件使用正确的 Design Token 和 `role="alert"`
...
- [ ] `CreateProjectDialog.tsx` L321：替换 `{lastError.code}: {lastError.message}` 渲染
- [ ] `KnowledgeGraphPanel.tsx` L917-918, L930, L1126, L1138：替换四处错误渲染
- [ ] `VersionHistoryContainer.tsx` L675-676, L727-728：替换合并错误和预览错误两处渲染
- [ ] 确认三个组件使用正确的 Design Token 和 `role="alert"`
...
- [ ] `JudgeSection.tsx` L18：`` `error (${state.error.code})` `` → `getHumanErrorMessage(state.error)`
- [ ] `JudgeSection.tsx` L54, L82, L85：`` `${res.error.code}: ${res.error.message}` `` → `getHumanErrorMessage(res.error)`
- [ ] `AiSettingsSection.tsx` L42, L104, L121：同上替换模式
- [ ] `DashboardPage.tsx` L428：同上替换模式
...
- [ ] `AiErrorCard.tsx` L768-770：将 `service_error` 状态下 `{error.errorCode}` 展示替换为 `getHumanErrorMessage({ code: error.errorCode, message: error.message })` 调用

**文件**: `components/features/AiDialogs/AiErrorCard.tsx`（修改）

### Task 2.6: 收口 CommandPalette 硬编码 + 新增 i18n key

- [ ] 在 `zh-CN.json` 中新增 `commandPalette.error.*` 命名空间，注册以下 key：
...
- [ ] 在 `en.json` 中新增相同结构的英文翻译
- [ ] 将 `CommandPalette.tsx` 中全部硬编码错误字符串替换为 `t("commandPalette.error.<场景>")` 调用
...
- [ ] 所有错误文字使用 `var(--color-text-error)` Token（不使用 `text-red-500` 等原始值）
- [ ] 带背景的错误区域使用 `var(--color-bg-error)` Token
- [ ] 所有错误区域设置 `role="alert"` 或 `aria-live="polite"`
- [ ] 错误文案的颜色对比度满足 WCAG 2.1 AA（4.5:1）
...
- [ ] 执行 `pnpm -C apps/desktop vitest run apps/desktop/tests/error-surface-closure.test.ts` 全局泄露检测通过
- [ ] 执行各组件测试文件全部通过
...
- [ ] 在 Storybook 或应用中手动触发至少 3 个组件的错误状态，截图确认展示人话文案
- [ ] 确认错误文案在暗色主题和亮色主题下均可读（Token 自动适配）

### Task 3.3: 清理确认

- [ ] 全局搜索确认 renderer 源码中无遗留的 `{error.code}` / `{error.message}` 直接渲染（排除 `errorMessages.ts` 和测试文件）
- [ ] 全局搜索确认无遗留的 `"ACTION_FAILED:` / `"NO_PROJECT:` 硬编码
- [ ] 确认所有新增 `getHumanErrorMessage` 导入指向 `renderer/src/lib/errorMessages.ts`
...
- [ ] 14 个泄露组件全部收口完成——无一处直接渲染 `error.code` 或 `error.message`
- [ ] CommandPalette 全部硬编码错误字符串替换为 i18n 调用
- [ ] `zh-CN.json` 和 `en.json` 包含 `commandPalette.error.*` 全量条目
- [ ] 所有错误展示使用语义化 Design Token
- [ ] 所有错误区域有正确的无障碍属性
- [ ] Storybook 构建通过
- [ ] PR body 包含 `Closes #988`
