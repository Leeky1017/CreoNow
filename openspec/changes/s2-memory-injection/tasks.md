...
- [ ] 1.1 审阅并确认 C13 边界：memory preview 注入 prompt。
- [ ] 1.2 审阅并确认职责分工：memory 服务提供 preview，settings fetcher 负责注入。
- [ ] 1.3 审阅并确认验收场景：有记忆注入 + context 端到端可见。
...
- [ ] 3.1 先写“有记忆注入”失败测试并确认 Red。
- [ ] 3.2 先写“异常降级”失败测试并确认 Red。
- [ ] 3.3 先写“端到端可见性”失败测试并确认 Red。

## 4. Green（最小实现通过）

- [ ] 4.1 仅实现使 Red 转绿的最小注入链路。
- [ ] 4.2 逐条使失败测试转绿，不引入无关功能。
- [ ] 4.3 复跑映射测试，确认 Scenario 对应测试全部通过。

## 5. Refactor（保持绿灯）

- [ ] 5.1 收敛 preview 读取与格式化注入逻辑，避免重复路径。
- [ ] 5.2 清理临时代码并保持外部契约不变。
- [ ] 5.3 回归执行后保持全绿。
