...
1. `version-control-p1-ai-mark-preview`（依赖已归档 `version-control-p0` + 已归档 `editor-p0`）
2. `editor-p2-diff-ai-collaboration`（依赖已归档 `editor-p0` + 已归档 `editor-p1` + 已归档 AI Service Phase 3）
3. `skill-system-p2-custom-skill-crud`（依赖已归档 `skill-system-p0` + 已归档 `skill-system-p1` + 已归档 AI Service Phase 3）

### 阶段 B — 中段推进（含跨泳道依赖）

4. `version-control-p2-diff-rollback`（依赖已归档 `version-control-p0` + `version-control-p1` + **跨泳道** `editor-p2`）
5. `editor-p3-zen-mode`（依赖已归档 `editor-p0` + 已归档 `editor-p1` + `editor-p2`）
6. `skill-system-p3-scheduler-concurrency-timeout`（依赖已归档 `skill-system-p0` + 已归档 `skill-system-p1` + `skill-system-p2`）
...
10. `version-control-p4-hardening-boundary`（依赖已归档 `version-control-p0` + `version-control-p1` + `version-control-p2` + `version-control-p3`）
