...
archive/workbench-p5-00-contract-sync (Phase A baseline)
    ...
    └──→ archive/workbench-p5-04-command-palette    ← 已完成并归档
               │
               ▼
...
- Change 01–03 与已归档 Change 04 无互相依赖。
- Change 05 依赖 Change 01–04 全部合并后方可进入 Red。
- 所有 Change 均在 Workbench 模块内，无跨模块泳道依赖。

## 依赖说明

- Change 01–03（以及已完成的 Change 04）进入 Red 前，必须完成对已归档 Change 00 delta spec 的 Dependency Sync Check。
