...
archive/workbench-p5-00-contract-sync (Phase A baseline)
    ...
    └──→ workbench-p5-04-command-palette            ← 依赖 00 的 IPC 文件列表通道确认
               │
               ▼
          workbench-p5-05-hardening-gate            ← 依赖 01/03/04 全部合并后
```

### 跨泳道依赖明细

- Change 01/03/04 无互相依赖，可并行执行。
- Change 05 依赖 Change 01/03/04 全部合并后方可进入 Red。
- 所有 Change 均在 Workbench 模块内，无跨模块泳道依赖。

## 依赖说明

- Change 01/03/04 进入 Red 前，必须完成对已归档 Change 00 delta spec 的 Dependency Sync Check。
- Change 05 进入 Red 前，必须完成对 Change 01/03/04 全部产出的 Dependency Sync Check。
