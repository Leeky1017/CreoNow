...
更新时间：2026-02-13 09:43

适用范围：`openspec/changes/` 下所有非 `archive/`、非 `_template/` 的活跃 change。

## 执行策略

- 当前活跃 change 数量为 **4**（Phase 2 Codex 上下文）。
- 执行模式：**两条并行泳道 + 依赖串行**。
...
- 已完成归档（Phase 2）：`p2-kg-context-level`（C8）、`p2-entity-matcher`（C10）。
...
2. **C11** `p2-fetcher-always`（0.5d）— rules fetcher → KG Always 注入（依赖 C8[已归档]）
3. **C12** `p2-fetcher-detected`（1d）— retrieved fetcher → Codex 引用检测（依赖 C10[已归档] + C11）

### 泳道 B — Memory 注入（可与泳道 A 并行）

4. **C13** `p2-memory-injection`（1d）— settings fetcher → Memory previewInjection（依赖 P1.C2）

### 推荐执行序列

```
C9 → C11 → C12（泳道 A：依赖串行；C10 已归档作为前置产物）
C13（泳道 B，可与泳道 A 并行）
```

C8/C10 已归档并作为上游前置；当前泳道 A 中，C11 依赖 C8 产物，C12 依赖 C10 + C11。

## 依赖关系总览

```text
C8 (kg-context-level, archived) ──┬──→ C10 (entity-matcher, archived) ──→ C12 (fetcher-detected)
C9 (kg-aliases) ──────────────────┘                                        ↑
