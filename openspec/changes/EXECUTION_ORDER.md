...
- 变更泳道：
  - Context Engine：`(p2 || p3) → p4`（`context-engine-p0` 与 `context-engine-p1` 已归档）
  - AI Service：`ai-service-p1 → p2 → (p3 || p4) → p5`（`ai-service-p0` 已归档）
  - Search & Retrieval：`(p1 || p2) → p3 → p4`（`search-retrieval-p0-fts-foundation` 已归档）

## 执行顺序

1. Phase A（并行）
   - `context-engine-p2-stable-prefix-hash`（依赖已归档 `context-engine-p1-token-budget-truncation`，已满足）
   - `context-engine-p3-constraints-rules-injection`（依赖已归档 `context-engine-p1-token-budget-truncation`，已满足）
   - `ai-service-p1-streaming-cancel-lifecycle`（依赖已归档 `ai-service-p0-llmproxy-config-security`）
2. Phase B（并行）
   - `ai-service-p2-panel-chat-apply-flow`（依赖 `ai-service-p1-streaming-cancel-lifecycle`）
   - `search-retrieval-p1-embedding-semantic-rag`（依赖 `search-retrieval-p0-fts-foundation`，已满足）
