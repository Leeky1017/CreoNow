...
- [ ] 1.1 审阅并确认需求边界（仅 Phase 1 止血：Token 清扫 + 原生元素替换 + Z-index 统一）
- [ ] 1.2 审阅并确认错误路径与边界路径（raw color/raw z-index/魔法阴影/散写原生元素的阻断与例外）
- [ ] 1.3 审阅并确认验收阈值与不可变契约（Feature 层禁止新增 raw color、数字 z-index、魔法阴影、散写原生 `button/input`）
...
- [ ] 3.1 为 `WB-P1-S1` 与 `WB-P1-S2` 新增失败测试，先验证当前代码库存在违例样本
- [ ] 3.2 为 `WB-P1-S4` 与 `WB-P1-S5` 新增失败测试，先验证魔法阴影与原生元素绕过问题
- [ ] 3.3 为 `WB-P1-S3` 新增叠层场景失败测试，先复现 token 漂移导致的 Z 轴风险

## 4. Green（最小实现通过）

- [ ] 4.1 仅实现使 `WB-P1-S1/S2` 转绿所需的最小替换（raw color/z-index → token）
- [ ] 4.2 仅实现使 `WB-P1-S4` 转绿所需的最小替换（魔法阴影 → `--shadow-*`）
- [ ] 4.3 仅实现使 `WB-P1-S5` 转绿所需的最小替换（散写原生元素 → Primitives）
- [ ] 4.4 逐条验证 `WB-P1-S3` 叠层顺序通过，不引入 Phase 2/3 非目标改动
