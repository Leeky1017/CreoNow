...
- [ ] 创建 `tests/visual/playwright.visual.config.ts`：
...
- [ ] 在 `package.json` 中注册 `test:visual` 命令

### Task 1.2: Storybook 服务 Fixture

**映射验收标准**: AC-1

- [ ] 创建 `visual.setup.ts`：启动 Storybook 静态服务或复用 `storybook:build` 产物
- [ ] 支持 CI 环境（无头浏览器）
...
- [ ] 创建 `primitives.visual.spec.ts`
- [ ] 遍历所有原语 Story（Button、Input、Card、Dialog 等 ≥20 个）
- [ ] 每个 Story 在 dark + light 两个主题下截图对比
- [ ] 生成初始 baseline 截图
...
- [ ] 创建 `layout.visual.spec.ts`
- [ ] 覆盖 AppShell、IconBar、Layout、Sidebar、StatusBar 等 ≥5 个
- [ ] dark + light 双主题
...
- [ ] 创建 `features.visual.spec.ts`
- [ ] 优先覆盖：DashboardPage、EditorPane、AiPanel、SettingsDialog、ExportDialog、CommandPalette、FileTreePanel、OutlinePanel、VersionHistoryPanel、SearchPanel
- [ ] dark + light 双主题
...
- [ ] 创建 `ContextMenu.stories.tsx`
- [ ] 创建 `DropdownMenu.stories.tsx`
- [ ] 创建 `ScrollArea.stories.tsx`

### Task 2.5: CI 集成

**映射验收标准**: AC-6, AC-7, AC-8

- [ ] 在 `.github/workflows/ci.yml` 新增 `visual-regression` job：
  ```yaml
  visual-regression:
  ...
  ```
- [ ] 将 `visual-regression` 加入 `ci` meta-job 的 `needs` 列表

### Task 2.6: 文档同步

**映射验收标准**: AC-10, AC-11

- [ ] 在 `docs/references/testing/` 新增视觉测试指南或在现有文档中补充章节
- [ ] 更新 `design-ui-architecture.md` 视觉测试说明
- [ ] 更新 `07-test-command-and-ci-map.md` 添加 `test:visual` 命令
