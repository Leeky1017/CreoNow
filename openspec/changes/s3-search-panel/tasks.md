...
- [ ] 1.1 审阅并确认 `s3-search-panel` 边界：仅完善搜索面板 UI 与跳转闭环，不改检索算法。
- [ ] 1.2 审阅并确认结果点击跳转、空态、错误态与加载态约束。
- [ ] 1.3 审阅并确认验收阈值：查询可用、结果可跳转、异常可见。
...
- [ ] S3-SEARCH-PANEL-S1 `输入查询后展示结果列表 [ADDED]`
...
- [ ] S3-SEARCH-PANEL-S2 `点击结果后跳转到目标文档位置 [ADDED]`
...
- [ ] S3-SEARCH-PANEL-S3 `空结果与错误态可区分显示 [MODIFIED]`
...
- [ ] 3.1 编写 S3-SEARCH-PANEL-S1 失败测试，确认当前查询链路不完整。
- [ ] 3.2 编写 S3-SEARCH-PANEL-S2 失败测试，确认点击结果后未形成稳定跳转闭环。
- [ ] 3.3 编写 S3-SEARCH-PANEL-S3 失败测试，确认空态/错误态呈现尚不满足要求。

## 4. Green（最小实现通过）

- [ ] 4.1 完善 SearchPanel 查询输入与结果渲染最小实现。
- [ ] 4.2 打通结果点击与编辑区跳转动作。
- [ ] 4.3 补齐空态、错误态、加载态并使 Red 用例转绿。

## 5. Refactor（保持绿灯）

- [ ] 5.1 统一 SearchPanel 状态机与渲染分支，减少重复分支逻辑。
- [ ] 5.2 复核 Workbench 模式一致性，避免局部引入新风格状态实现。
