...
- [ ] 测试：`polish` skill，输入 500 字、输出 480 字纯文本，断言 `validateSkillRunOutput()` 返回 `{ ok: true }`
- [ ] 测试：`rewrite` skill，输入 300 字、输出 350 字纯文本，断言校验通过
- [ ] 测试：`continue` skill，输入上下文 200 字、输出 1,500 字纯文本，断言校验通过
- [ ] 测试：`expand` skill，输入 100 字、输出 800 字纯文本，断言校验通过
...
- [ ] 测试：输出为空字符串 `""`，断言返回 `{ ok: false, error: { code: "SKILL_OUTPUT_INVALID" } }`，message 包含 `"空内容"`
- [ ] 测试：输出为仅含空白 `"   \n  \t  "`，断言同样被拦截
- [ ] 测试：输出为 `undefined`，断言被拦截
...
- [ ] 测试：输出包含 ` ``` ` 但无语言标识（裸代码块），断言同样被拦截
- [ ] 测试：输出包含单个反引号（`inline code`）而非三个，断言通过（不误伤 inline code）
...
- [ ] 测试：输出包含 `<div>text</div>`，断言被拦截且 message 包含 `"HTML"`
- [ ] 测试：输出包含 `<script>alert(1)</script>`，断言被拦截
- [ ] 测试：输出包含 `<p class="x">段落</p>`，断言被拦截
- [ ] 测试：输出包含中文书名号 `《红楼梦》` 或数学不等式 `a < b`，断言通过（不误伤非 HTML 尖括号）
...
- [ ] 测试：`polish`，输入 200 字，输出 2,500 字（12.5 倍），断言被拦截
- [ ] 测试：`polish`，输入 200 字，输出 1,800 字（9 倍），断言通过
- [ ] 测试：`rewrite`，输入 100 字，输出 1,100 字（11 倍），断言被拦截
- [ ] 测试：`continue`，输入 300 字，输出 4,000 字（13.3 倍），断言通过（未超 20 倍）
- [ ] 测试：`continue`，输入 300 字，输出 7,000 字（23.3 倍），断言被拦截
- [ ] 测试：`expand`，输入 100 字，输出 2,100 字（21 倍），断言被拦截
...
- [ ] 测试：`synopsis` skill 的输出仍走 `validateSynopsisOutput()` 函数，断言行为与变更前一致
- [ ] 测试：确认 `validateSkillRunOutput()` 对 `synopsis` 的分支逻辑未改变
...
- [ ] 测试：`polish`，`inputText` 为空字符串，输出 5,000 字，断言跳过膨胀检测，仅执行格式校验，最终通过
- [ ] 测试：`polish`，`inputText` 为 `undefined`，输出 5,000 字，断言同上
...
- [ ] 在 `ipc-contract.ts` 的 `IpcErrorCode` 联合类型中注册 `"SKILL_OUTPUT_INVALID"`

**文件**: `apps/desktop/main/src/ipc/contract/ipc-contract.ts`（修改）

### Task 2.2: 实现通用输出校验函数

- [ ] 创建 `validateCreativeSkillOutput(args: { skillId: string; outputText?: string; inputText?: string }): ServiceResult<true>` 函数
- [ ] 按规则顺序实现 V-EMPTY → V-CODEBLOCK → V-HTML → V-INFLATE-* 校验链
- [ ] 膨胀阈值根据 skill ID 选择 10 倍或 20 倍
- [ ] 输入基准为空时跳过膨胀检测
...
- [ ] 将 `if (leafSkillId(args.skillId) !== "synopsis") return { ok: true }` 替换为按 skill ID 分发：
...
- [ ] 确保 `inputText` 从调用参数中正确传递到校验函数

**文件**: `apps/desktop/main/src/services/skills/skillExecutor.ts`（修改）

### Task 2.4: 错误消息人话映射

- [ ] 在 `errorMessages.ts` 中为 `SKILL_OUTPUT_INVALID` 添加人话映射：「AI 输出异常，请重新尝试」
...
- [ ] 确认 synopsis 测试（`synopsisSkill.execution.test.ts`）仍全绿
- [ ] 确认 HTML 正则不误伤书名号 `《》` 和数学不等式 `<` `>`
- [ ] 确认错误码已纳入 `IpcErrorCode` 类型，TypeScript strict 编译通过
- [ ] 确认无冗余导入或临时文件
...
- [ ] 所有 Scenario（S-OUT-1 到 S-OUT-9）有对应测试且通过
- [ ] PR body 包含 `Closes #985`
- [ ] 审计评论闭环完成（PRE-AUDIT → RE-AUDIT → FINAL-VERDICT）
- [ ] TypeScript strict 编译通过，无新增 `any` 类型
- [ ] `synopsisSkill.execution.test.ts` 已有测试全绿，无回归
