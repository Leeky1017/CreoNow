...
- [x] 1.1 审阅并确认需求边界（聚焦：Token 预算估算共享化）
...
| -------------- | ---------------------------------------------------------------- | ---------------------------------------- |
| CE-AUD-M2-S1   | `apps/desktop/tests/unit/context/token-budget-shared-helper.test.ts` | `S1: token estimation/truncation must use shared helper...`（注释） |
