...
- [x] 1.1 审阅并确认需求边界（聚焦：memory/document 服务拆分第二阶段）
- [x] 1.2 审阅并确认错误路径与边界路径
- [x] 1.3 审阅并确认验收阈值与不可变契约
- [x] 1.4 若存在上游依赖，先完成依赖同步检查（Dependency Sync Check）并记录“无漂移/已更新”；无依赖则标注 N/A（本 change 依赖：aud-h6a-main-service-decomposition）
...
Evidence pointers（Wave3）：

- RUN_LOG：`openspec/_ops/task_runs/ISSUE-595.md`
  - Red/Green：见 `## Runs`（Wave3 TDD Red / Green / 回归门禁）
