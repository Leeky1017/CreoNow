...
- [x] 1.1 审阅并确认 C9 边界：`aliases` 字段读写、UI 编辑与边界测试。
- [x] 1.2 审阅并确认不可变契约：不扩展 matcher/fetcher 行为。
- [x] 1.3 审阅并确认边界清单：空、超长、重复 alias。
...
- [x] 3.1 先写 aliases 创建与读取失败测试并确认 Red。
- [x] 3.2 先写 aliases 更新失败测试并确认 Red。
- [x] 3.3 先写边界场景失败测试并确认 Red。

## 4. Green（最小实现通过）

- [x] 4.1 仅实现使 Red 转绿的最小改动（写路径 + UI 绑定）。
- [x] 4.2 逐条使失败测试转绿，不引入无关功能。
- [x] 4.3 复跑映射测试，确认 Scenario 对应测试全部通过。

## 5. Refactor（保持绿灯）

- [x] 5.1 收敛 aliases 标准化/校验逻辑到单一路径。
- [x] 5.2 清理临时代码，保持外部契约不变。
- [x] 5.3 回归执行后保持全绿。
