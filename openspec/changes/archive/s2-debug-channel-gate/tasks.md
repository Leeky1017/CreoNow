...
- [x] 1.1 审阅并确认需求边界：仅处理 `db:debug:tablenames` 的生产门禁
- [x] 1.2 审阅并确认错误路径与边界路径：防止生产环境调试通道泄露结构信息
- [x] 1.3 审阅并确认验收阈值：production 禁用、non-production 可用
...
- [x] 3.1 为 `IPC-S2-DBG-S1` 编写失败测试，先证明当前生产环境可暴露 debug 通道
- [x] 3.2 为 `IPC-S2-DBG-S2` 编写失败测试，先锁定非生产环境可用性契约
- [x] 3.3 运行对应测试并记录 Red 证据（失败输出与 Scenario ID 对应）

## 4. Green（最小实现通过）

- [x] 4.1 仅实现让 `IPC-S2-DBG-S1/S2` 转绿的最小改动（注册阶段环境门禁）
- [x] 4.2 保持现有非生产调试链路行为不变

## 5. Refactor（保持绿灯）

- [x] 5.1 收敛环境判断逻辑，避免同一通道出现多处不一致门禁
- [x] 5.2 清理重复分支，保持注册链路可读性
