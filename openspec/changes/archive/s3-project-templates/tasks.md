...
- [x] 1.1 审阅并确认 `s3-project-templates` 边界：仅交付模板服务、内置模板资源与创建流程接入。
- [x] 1.2 审阅并确认模板 schema 与 `project:create` 既有契约映射关系。
- [x] 1.3 审阅并确认验收阈值：模板可选、可应用、创建产物结构可验证。
...
- [x] S3-PROJECT-TPL-S1 `新建项目可选择内置模板并生成结构化初始内容 [ADDED]`
...
- [x] S3-PROJECT-TPL-S2 `模板应用后保持项目创建对外契约不变 [MODIFIED]`
...
- [x] S3-PROJECT-TPL-S3 `非法自定义模板输入会被校验并返回明确错误 [ADDED]`
...
- [x] 3.1 编写 S3-PROJECT-TPL-S1 失败测试，确认当前创建流程未应用模板结构。
- [x] 3.2 编写 S3-PROJECT-TPL-S2 失败测试，确认模板接入后契约稳定性防线尚未建立。
- [x] 3.3 编写 S3-PROJECT-TPL-S3 失败测试，确认非法模板输入校验不足。

## 4. Green（最小实现通过）

- [x] 4.1 实现模板服务最小能力并接入四类内置模板。
- [x] 4.2 打通创建流程模板选择与应用路径，使 S1/S2 转绿。
- [x] 4.3 增加模板 schema 校验并通过 S3。

## 5. Refactor（保持绿灯）

- [x] 5.1 收敛模板应用流程，避免多层 helper 转发导致过度抽象。
- [x] 5.2 去重模板字段映射逻辑，保持项目管理模块命名与组织一致。
