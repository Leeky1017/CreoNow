...
- 当本地模型加载失败或 API 不可达时，`embedding:semantic:search` 返回可判定降级状态。
...
- **当** 用户在语义模式输入查询文本
- **则** 系统通过 `embedding:semantic:search` 返回按相似度排序的段落
...
- `rag:context:retrieve` 成功时返回 `chunks[]`（含 `chunkId/documentId/text/score/tokenEstimate`）。
...
- **假设** 查询与项目内容无语义关联
- **当** `rag:context:retrieve` 未命中超过 `minScore` 的 chunk
- **则** 返回空召回集合
