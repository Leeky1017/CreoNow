...
- [x] 1.1 审阅并确认需求边界：仅修复 `kgStore/searchStore` 请求竞态覆盖问题
- [x] 1.2 审阅并确认错误路径与边界路径：防止旧请求回写覆盖最新状态
- [x] 1.3 审阅并确认验收阈值：快速切换/快速输入场景下仅最新请求可提交结果
...
- [x] 3.1 编写 `WB-S2-SRF-S1` 失败测试，先证明项目快速切换存在旧结果覆盖
- [x] 3.2 编写 `WB-S2-SRF-S2` 失败测试，先证明快速输入存在旧结果反写
- [x] 3.3 运行对应测试并记录 Red 证据（失败输出与 Scenario ID 对应）

## 4. Green（最小实现通过）

- [x] 4.1 仅实现让 `WB-S2-SRF-S1/S2` 转绿的最小改动（请求标记/中断 + 提交前校验）
- [x] 4.2 保持 store 对外接口不变，仅收敛内部并发写入策略

## 5. Refactor（保持绿灯）

- [x] 5.1 统一请求标记命名与提交流程，降低未来竞态复发概率
- [x] 5.2 清理重复并发防护逻辑，保持 store 内部职责清晰
