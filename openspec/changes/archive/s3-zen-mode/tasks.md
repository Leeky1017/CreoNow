...
- [x] 1.1 审阅并确认 `s3-zen-mode` 边界：仅实现禅模式显隐与布局恢复，不扩展编辑器功能。
- [x] 1.2 审阅并确认进入/退出禅模式的状态切换与快捷键约束。
- [x] 1.3 审阅并确认验收阈值：进入可隐藏干扰项、退出可恢复原布局。
...
- [x] S3-ZEN-MODE-S1 `进入禅模式时隐藏侧栏并聚焦编辑器 [ADDED]`
...
- [x] S3-ZEN-MODE-S2 `退出禅模式后恢复进入前布局状态 [ADDED]`
...
- [x] S3-ZEN-MODE-S3 `快捷键切换遵循现有布局状态契约 [MODIFIED]`
...
- [x] 3.1 编写 S3-ZEN-MODE-S1 失败测试，确认进入态未形成完整隐藏行为。
- [x] 3.2 编写 S3-ZEN-MODE-S2 失败测试，确认退出后恢复约束不满足。
- [x] 3.3 编写 S3-ZEN-MODE-S3 失败测试，确认快捷键路径与状态同步尚未收敛。

## 4. Green（最小实现通过）

- [x] 4.1 实现进入禅模式最小行为（隐藏侧栏/面板并聚焦编辑区）。
- [x] 4.2 实现退出禅模式恢复逻辑（恢复进入前布局快照）。
- [x] 4.3 接入快捷键触发并保证三条 Red 用例转绿。

## 5. Refactor（保持绿灯）

- [x] 5.1 收敛禅模式状态与 layout store 交互，避免并行状态源。
- [x] 5.2 清理重复切换逻辑，保持进入/退出路径可读且可测试。
