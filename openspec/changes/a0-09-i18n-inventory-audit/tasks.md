...
- [ ] 测试：定义 `07` §三已知裸字符串清单（至少以下条目），每条含文件路径和字符串内容：
...
- [ ] 测试：断言扫描结果中每条已知裸字符串均可匹配（文件路径包含 + 内容包含）
- [ ] 测试：断言未覆盖条目数 === 0
...
- [ ] 测试：断言扫描结果中每条记录包含 `module`、`filePath`、`line`、`rawString`、`suggestedKey`、`priority` 六个字段
- [ ] 测试：断言 `priority` 值仅为 `"P0"` 或 `"P1"`
- [ ] 测试：断言 `module` 值为合法模块名（`editor` / `version-control` / `ai-service` / `workbench` / ...）
...
- [ ] 测试：准备包含 Tailwind class、console.log 调试文案、测试文件字符串的 mock 输入
- [ ] 测试：断言扫描器对 Tailwind class `"flex items-center"` 返回排除结果
- [ ] 测试：断言扫描器对 `console.error("debug info")` 内的字符串返回排除结果
- [ ] 测试：断言扫描器对 `*.test.tsx` 文件中的字符串返回排除结果
...
- [ ] 测试：记录扫描前 `renderer/src/` 和 `renderer/src/i18n/locales/` 目录下全部文件的 hash
- [ ] 测试：执行扫描
- [ ] 测试：断言扫描后所有文件 hash 与扫描前一致（无修改）
...
- [ ] 递归遍历 `renderer/src/` 下全部 `.tsx` / `.ts` 文件
- [ ] 解析 JSX 文本节点：匹配 `>text<` 中的裸文本（排除 `{expression}` 和 `{t("key")}`）
- [ ] 解析字符串属性：匹配 `title="..."`, `placeholder="..."`, `aria-label="..."`, `alt="..."` 等用户可见属性中的裸字符串
- [ ] 解析逻辑层文案：匹配函数调用中面向用户的文案常量（error message、tooltip、status text）
- [ ] 每条结果输出：`{ module, filePath, line, rawString, suggestedKey, priority }`

**文件**: `apps/desktop/renderer/src/utils/i18n-inventory-scanner.ts`（新建）或作为脚本 `scripts/i18n-inventory-scan.ts`（新建）

### Task 2.2: 实现排除规则

- [ ] 排除 CSS class name / Tailwind utility（匹配模式：`className="..."`, `cn(...)` 内的字符串）
- [ ] 排除 DOM 事件名和 HTML 标签名
- [ ] 排除技术常量（路由路径 `/...`、存储 key `STORAGE_KEY_*`、正则表达式）
- [ ] 排除 `console.log` / `console.error` / `console.warn` / `console.info` 中的参数
- [ ] 排除测试文件（`*.test.ts` / `*.test.tsx` / `*.spec.ts`）
- [ ] 排除已走 `t()` 的调用：`t("...")` 内的 key 字符串不计入裸字符串
...
- [ ] 对 `renderer/src/` 执行完整扫描
- [ ] 输出 Markdown 格式清理清单，按模块分组、按文件路径 + 行号升序排列
- [ ] P0 条目排在每组前面
- [ ] 将清理清单保存为 `openspec/changes/a0-09-i18n-inventory-audit/i18n-inventory-checklist.md`

**文件**: `openspec/changes/a0-09-i18n-inventory-audit/i18n-inventory-checklist.md`（新建）

### Task 2.5: 对照 `07` §三验证覆盖率

- [ ] 逐条核对 `07` §三中列出的裸字符串是否全部出现在清理清单中
- [ ] 若发现遗漏，检查原因（文件路径变更 / 排除规则误伤 / 已被修复）并补充到清单
- [ ] 确认覆盖缺失数 = 0
...
- [ ] 运行 `pnpm -C apps/desktop vitest run`，确认扫描器测试全部通过
- [ ] 运行 `pnpm -C apps/desktop tsc --noEmit`，确认无类型错误
- [ ] 确认 `renderer/src/` 和 locale 文件均未被修改（只读约束）

### Task 3.2: 清理清单可用性验证

- [ ] 确认清理清单 Markdown 格式正确、表格可渲染
- [ ] 确认清单中编辑器模块条目 ≥ 6 条（对应 `07` §三编辑器区域已知裸字符串）
- [ ] 确认清单中版本历史模块条目 ≥ 8 条（对应 `07` §三版本历史已知裸字符串）
- [ ] 确认清单中 AI 面板模块条目 ≥ 3 条（对应 `07` §三 AI 面板已知裸字符串）
- [ ] 确认 A0-16 开发 Agent 可直接从清单中领取 editor / version-control 模块的修复任务
