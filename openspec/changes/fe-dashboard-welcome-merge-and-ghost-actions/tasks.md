...
- [ ] 1.4 依赖同步检查（Dependency Sync Check）：
  - [ ] `fe-cleanup-proxysection-and-mocks` 已合并（避免 dead 入口堆叠）— **STOP if pending**
  - [ ] `fe-ui-open-folder-entrypoints` 已合并（Open Folder 行为闭环）— **STOP if pending**
...
- [ ] 3.1 `PM-FE-DASH-S1`：mock projectStore 为空，渲染 AppShell，断言渲染 DashboardPage 空状态而非 WelcomeScreen。
  - 期望红灯原因：当前 AppShell L622-623 无项目时渲染 WelcomeScreen。
- [ ] 3.2 `PM-FE-DASH-S2/S2b`：渲染 DashboardPage 空状态，断言有 "Create Project" + "Open Folder" 按钮且触发 IPC。
  - 期望红灯原因：当前 `dashboard-empty` 区域无这两个按钮。
- [ ] 3.3 `PM-FE-DASH-S3`：渲染 DashboardPage，遍历所有 button/a，断言每个都有 onClick/href。
  - 期望红灯原因：当前可能存在无 handler 的幽灵按钮。
- [ ] 3.4 `PM-FE-DASH-S4`：断言 `features/welcome/WelcomeScreen.tsx` 文件不存在。
  - 期望红灯原因：当前文件存在。
- 运行：`pnpm -C apps/desktop test:run features/dashboard/Dashboard.empty-state` / `DashboardPage.ghost-buttons.guard`

## 4. Green（最小实现通过）

- [ ] 4.1 `AppShell.tsx` L620-624：删除 WelcomeScreen 分支，改为始终渲染 DashboardPage（由 DashboardPage 自身处理空状态） → S1 转绿
- [ ] 4.2 删除 `features/welcome/WelcomeScreen.tsx` + `WelcomeScreen.stories.tsx` → S4 转绿
- [ ] 4.3 `DashboardPage.tsx` L560-566 空状态：新增 "Create Project" + "Open Folder" 按钮，onClick 闭环 → S2/S2b 转绿
- [ ] 4.4 `DashboardPage.tsx` 全文件：移除所有无 handler 的幽灵按钮（或补齐 handler） → S3 转绿

## 5. Refactor（保持绿灯）

- [ ] 5.1 去重空状态组件，确认 DashboardPage 空状态与 Onboarding Step 3 的 Open Folder 入口复用同一 action
- [ ] 5.2 确认删除 WelcomeScreen 后无其他引用断裂
...
- [ ] 6.4 记录 Dependency Sync Check：`fe-cleanup-proxysection-and-mocks` + `fe-ui-open-folder-entrypoints` 合并状态
