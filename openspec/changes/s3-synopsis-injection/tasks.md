...
- [ ] 1.1 审阅并确认 `s3-synopsis-injection` 范围：摘要入库、检索与续写注入
- [ ] 1.2 审阅并确认失败路径：存储失败、检索失败、空摘要项目的降级行为
- [ ] 1.3 审阅并确认不可变契约：续写主流程可用、层组装优先级与预算策略不被破坏
...
- [ ] S3-SYN-INJ-S2 `无摘要数据时续写流程保持正常 [ADDED]`
...
- [ ] S3-SYN-INJ-S3 `存储或检索失败时返回结构化降级信号 [ADDED]`
...
- [ ] 3.1 编写 S3-SYN-INJ-S1 失败测试，确认当前未实现稳定注入
- [ ] 3.2 编写 S3-SYN-INJ-S2 失败测试，确认无摘要场景未覆盖
- [ ] 3.3 编写 S3-SYN-INJ-S3 失败测试，确认失败可观测语义缺失
- [ ] 3.4 运行最小测试集并记录 Red 证据

## 4. Green（最小实现通过）

- [ ] 4.1 实现 `synopsisStore` 表结构与存取接口
- [ ] 4.2 实现 `synopsisFetcher` 并注册到 layer 组装链路
- [ ] 4.3 建立失败时结构化降级输出，确保续写主流程可用

## 5. Refactor（保持绿灯）

- [ ] 5.1 收敛摘要注入组装逻辑，去除重复转换和透传 helper
- [ ] 5.2 复核注入顺序与 token 预算裁剪策略的一致性

## 6. Evidence

- [ ] 6.1 记录 RUN_LOG（含 Red 失败证据、Green 通过证据与关键命令输出）
- [ ] 6.2 记录 Dependency Sync Check 的输入、核对结论与后续动作（无漂移/已更新）
- [ ] 6.3 记录摘要入库、检索、注入与失败降级的验证证据
