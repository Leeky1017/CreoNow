...
- [ ] 测试：构造 2 MB 大小的 `contentJson`，调用 `file:document:save` 处理函数，断言返回 `{ ok: true }`
- [ ] 测试：构造 7.3 MB 大小的 `contentJson`，调用 `file:document:save` 处理函数，断言返回 `{ ok: false, error: { code: "DOCUMENT_SIZE_EXCEEDED" } }`
- [ ] 测试：断言返回的 error.message 中包含当前文档大小信息（如 `"7.3 MB"`）
...
- [ ] 测试：构造恰好 5,242,880 字节的 `contentJson`，断言保存成功（`{ ok: true }`）
- [ ] 测试：构造 5,242,881 字节的 `contentJson`，断言保存被拦截（`{ ok: false, error: { code: "DOCUMENT_SIZE_EXCEEDED" } }`）
...
- [ ] 测试：构造 6 MB 大小的 `contentJson`，`reason` 设为 `'autosave'`，调用保存处理函数，断言返回 `{ ok: false, error: { code: "DOCUMENT_SIZE_EXCEEDED" } }`
- [ ] 测试：确认自动保存请求与手动保存请求使用相同的体积校验逻辑
...
- [ ] 测试：直接调用 `documentCoreService.save()` 传入 6 MB 的 `contentJson`，断言返回体积超限错误
- [ ] 测试：直接调用 `documentCoreService.save()` 传入 2 MB 的 `contentJson`，断言正常保存
...
- [ ] 测试：构造纯中文字符组成的 `contentJson`（中文字符 UTF-8 编码每字 3 字节），验证体积计算使用字节而非字符数——例如 1,747,627 个中文字符（约 5 MB UTF-8）应被拦截，即使 `string.length` 仅约 1.7M
- [ ] 测试：构造相同字符数的纯 ASCII 字符 `contentJson`，验证 ASCII 场景下 `string.length` 与字节数一致时仍走 `Buffer.byteLength` 逻辑
...
- [ ] 在合适的共享位置定义 `MAX_DOCUMENT_SIZE_BYTES = 5 * 1024 * 1024`
- [ ] 在 `ipc-contract.ts` 的 `IpcErrorCode` 联合类型中注册 `"DOCUMENT_SIZE_EXCEEDED"`

**文件**: `apps/desktop/main/src/ipc/contract/ipc-contract.ts`（修改）

### Task 2.2: IPC 层实施体积校验

- [ ] 在 `file:document:save` 处理函数中，参数校验通过后、调用 `documentCoreService.save()` 之前，增加体积校验逻辑
- [ ] 校验逻辑：`if (Buffer.byteLength(payload.contentJson, 'utf-8') > MAX_DOCUMENT_SIZE_BYTES)` 返回 `DOCUMENT_SIZE_EXCEEDED` 错误
- [ ] 错误消息包含当前文档大小（MB，保留一位小数）

**文件**: `apps/desktop/main/src/ipc/file.ts`（修改）

### Task 2.3: Service 层实施体积校验

- [ ] 在 `documentCoreService.save()` 的事务开启前，增加同样的体积校验
- [ ] 返回 Service 级别的错误结果
...
- [ ] 在 `errorMessages.ts` 中为 `DOCUMENT_SIZE_EXCEEDED` 添加人话映射：「文档内容过大，请精简后重试」
...
- [ ] 确认体积常量在 IPC 层和 Service 层共享同一定义，无重复硬编码
- [ ] 确认无残留 `string.length` 形式的体积判断
...
- [ ] 所有 Scenario（S-SIZE-1 到 S-SIZE-6）有对应测试且通过
- [ ] PR body 包含 `Closes #984`
