...
- [ ] 1.1 审阅并确认需求边界（Phase 2 仅覆盖 AppShell 拆分、Shell 权限边界、IPC 收敛）
- [ ] 1.2 审阅并确认错误路径与边界路径（布局越权、面板挤压、IPC 调用绕过 service）
- [ ] 1.3 审阅并确认验收阈值与不可变契约（Feature 禁止接管 viewport；renderer 禁止直接 IPC）
