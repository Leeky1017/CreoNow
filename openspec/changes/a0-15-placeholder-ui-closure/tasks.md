...
- [ ] 测试：渲染 SettingsAccount 页，断言所有操作按钮处于 disabled 状态
- [ ] 测试：hover disabled 按钮，断言 tooltip 文本匹配 `t('settings.account.comingSoonTooltip')` 的值
- [ ] 测试：点击 disabled 按钮，断言无事件触发
...
- [ ] 测试：渲染 Search 面板，断言不存在 "View More" 文本节点
- [ ] 测试：渲染 Search 面板，断言不存在 "Search All Projects" 文本节点
- [ ] 测试：搜索结果列表正常渲染，不受隐藏影响
...
- [ ] 测试：渲染 ChatHistory 列表，点击某条历史记录，断言不调用任何回调
- [ ] 测试：渲染 ChatHistory 列表，点击某条历史记录，断言 `console.info` 未被调用
- [ ] 测试：hover 历史记录条目，断言 tooltip 文本匹配 `t('common.comingSoon')` 的值
...
- [ ] 测试：渲染版本历史面板，断言 Restore 按钮处于 disabled 状态
- [ ] 测试：hover Restore 按钮，断言 tooltip 文本匹配 `t('versionControl.restoreComingSoon')` 的值
...
- [ ] 测试：`zh-CN.json` 包含 `common.comingSoon`、`common.featureInDevelopment`、`settings.account.comingSoonTooltip`、`versionControl.restoreComingSoon` 四个 key
- [ ] 测试：`en.json` 包含相同的四个 key
- [ ] 测试：中英文文件中新增 key 的数量一致
...
- [ ] 修改 `SettingsAccount.tsx`，为每个 disabled 按钮添加 Radix UI `Tooltip`，文案为 `t('settings.account.comingSoonTooltip')`
- [ ] 确认 `aria-disabled="true"` 已设置
- [ ] 确认 tooltip 通过 `aria-describedby` 关联

**文件**: `apps/desktop/renderer/src/features/settings-dialog/SettingsAccount.tsx`

### Task 2.2: Search 面板隐藏无功能链接

- [ ] 在 Search 面板组件中移除 "View More" 链接的 JSX 渲染（条件渲染 `false` 或移除代码）
- [ ] 移除 "Search All Projects" 链接的 JSX 渲染
- [ ] 确认搜索结果列表不受影响
- [ ] 在被移除的 JSX 代码位置添加注释 `// placeholder: hidden in v0.1, restore when search expansion is implemented`

**文件**: Search 面板组件（需搜索确认具体文件）

### Task 2.3: ChatHistory 交互降级

- [ ] 修改 ChatHistory 组件，将 `console.info("TODO")` 回调替换为 disabled 状态 + tooltip
- [ ] 条目视觉上标记为不可点击（cursor 不变为 pointer）
- [ ] 添加 tooltip `t('common.comingSoon')`

**文件**: `apps/desktop/renderer/src/features/rightpanel/` 下的 ChatHistory 组件

### Task 2.4: 版本恢复 Restore 按钮添加 tooltip

- [ ] 为 Restore 按钮的 disabled 状态添加 tooltip `t('versionControl.restoreComingSoon')`
- [ ] 确认 `aria-disabled="true"` 已设置
...
- [ ] 确认受影响组件在 Storybook 中可构建（`pnpm -C apps/desktop storybook:build`）
- [ ] 确认 Settings Account 页的 Coming Soon tooltip 在 Story 中正确显示
- [ ] 确认 Search 面板在 Story 中不显示隐藏链接

### Task 3.2: 全局排查

- [ ] 搜索代码库中其他可能的占位 UI（`grep -rn "console.info\|TODO\|FIXME\|placeholder" renderer/src/`），确认处置清单是否有遗漏
- [ ] 若发现新的占位 UI，记录但不在本任务中处置——上报给 Owner
