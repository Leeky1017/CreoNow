...
- [x] 1.1 审阅 `unified-roadmap` 中 AR-C14 的范围、边界与依赖
- [x] 1.2 审阅 `skill-system` 主 spec 中内置技能与加载约束
...
- [x] 2.2 为测试标注 Scenario ID（S2-WS-1/2）并建立追踪关系
- [x] 2.3 设定门禁：未出现 Red（失败测试）不得进入实现

## 3. Red（先写失败测试）

- [x] 3.1 编写“5 个写作技能均可被 loader 发现”的失败测试
- [x] 3.2 编写“技能文档字段缺失时返回可诊断失败”的失败测试
- [x] 3.3 运行目标测试并记录 Red 失败证据

## 4. Green（最小实现通过）

- [x] 4.1 补齐 write/expand/describe/shrink/dialogue 的 SKILL.md
- [x] 4.2 仅实现让 Red 转绿所需的最小改动
- [x] 4.3 复跑映射测试并确认全部转绿

## 5. Refactor（保持绿灯）

- [x] 5.1 统一写作技能文档结构与命名，去除重复描述
- [x] 5.2 清理无关噪音文本，保持对外契约不变
