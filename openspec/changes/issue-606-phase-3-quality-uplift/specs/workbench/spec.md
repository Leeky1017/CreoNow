...
#### Scenario: 侧栏溢出区域统一使用 ScrollArea [ADDED]
...
#### Scenario: transition-all 收敛为属性级过渡 [ADDED]
...
#### Scenario: 过渡时长与缓动统一引用 token [ADDED]
...
#### Scenario: 系统启用 reduced motion 时动画自动降级 [ADDED]
...
#### Scenario: 键盘导航与 focus-visible 指示一致 [ADDED]
...
#### Scenario: 视觉回归门禁拦截非预期样式漂移 [ADDED]
