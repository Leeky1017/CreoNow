...
- [x] 3.2 `AI-FE-GUIDE-S2`：mock UPSTREAM_ERROR，断言引导卡片含 Settings 跳转。
  - 期望红灯原因：当前无分流引导。
...
- [x] 4.2 AiPanel：UPSTREAM_ERROR / AI_NOT_CONFIGURED 分支渲染引导卡片（Settings → AI 跳转按钮） → S2 转绿
- [x] 4.3 AiPanel L1587-1620：删除内联 `<style>`，`@keyframes blink` 迁移到 main.css（含 reduced-motion） → S（STYLE）转绿

## 5. Refactor（保持绿灯）

- [x] 5.1 抽取 `ErrorGuideCard` 为可复用组件（统一 DB/Provider 两类引导 UI）
- [ ] 5.2 降噪：减少 header/content/footer 多层 border（本次仅完成引导与样式注入治理，降噪在后续 `fe-visual-noise-reduction` 收口）
