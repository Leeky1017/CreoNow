...
- [ ] **Q1**: 搜索 QualityPanel 质量检查按钮的渲染逻辑，确认按钮是否已渲染、绑定了什么点击事件
  - 命令：`grep -rn "QualityPanel\|quality.*check\|质量检查" apps/desktop/renderer/src/`
- [ ] **Q6**: 分析 QualityPanel 组件的完整功能列表——除质量检查外还有什么？哪些可独立使用？
...
- [ ] **Q2**: 追踪质量检查的 IPC 调用链路——从 QualityPanel 的点击事件 → IPC 通道 → JudgeService 方法
  - 命令：`grep -rn "judge" apps/desktop/main/src/ipc/`
  - 命令：`grep -rn "judge" apps/desktop/preload/`
- [ ] **Q3**: 阅读 `JudgeService.ensure()` 方法的源码，确认返回 `MODEL_NOT_READY` 的条件逻辑
...
- [ ] 自检 3：LLM Proxy 就绪度已明确回答——"当前能/不能支撑 Judge 调用" ✓/✗
- [ ] 自检 4：与 A0-15 占位 UI 策略一致性已确认 ✓/✗
- [ ] 自检 5：决策结论格式可被后续执行任务直接消费 ✓/✗
